...
import app.tzvpn.data.local.datastore.PreferencesDataStore
import app.tzvpn.data.local.datastore.SplitTunnelingMode
import app.tzvpn.data.local.datastore.SshCipher
import app.tzvpn.tunnel.DomainRouter
import app.tzvpn.tunnel.GeoBypassCountry
import app.tzvpn.tunnel.GeoBypassData
...
import app.tzvpn.tunnel.HevSocks5Tunnel
import app.tzvpn.tunnel.HttpProxyServer
import app.tzvpn.tunnel.NaiveBridge
import app.tzvpn.tunnel.NaiveSocksBridge
import app.tzvpn.tunnel.RateLimiter
import app.tzvpn.tunnel.SlipstreamBridge
import app.tzvpn.tunnel.SlipstreamSocksBridge
import app.tzvpn.tunnel.Socks5ProxyBridge
import app.tzvpn.tunnel.VlessBridge
import app.tzvpn.tunnel.SnowflakeBridge
import app.tzvpn.tunnel.SshTunnelBridge
import app.tzvpn.tunnel.TorSocksBridge
...
@AndroidEntryPoint
class TZVPNVpnService : VpnService() {

    companion object {
        private const val TAG = "TZVPNVpnService"
        const val ACTION_CONNECT = "app.tzvpn.CONNECT"
        const val ACTION_DISCONNECT = "app.tzvpn.DISCONNECT"
        const val ACTION_RECONNECT = "app.tzvpn.RECONNECT"
    ...
    @Inject
    lateinit var chainRepository: app.tzvpn.domain.repository.ChainRepository
    ...
    private fun connect(profileId: Long) {
        ...
        connectJob = serviceScope.launch {
            ...
            app.tzvpn.util.AppLog.redactSensitive = profile.isLocked
            ...
            if (wakeLock == null) {
                wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "TZVPN:VpnWakeLock").apply {
            ...
            if (wifiLock == null) {
                ...
                wifiLock = wifiManager?.createWifiLock(wifiMode, "TZVPN:VpnWifiLock")?.apply {
                    setReferenceCounted(false)
                    acquire()
                }
            ...
            try {
                ...
                val globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? =
                ...
                when (currentTunnelType) {
                    ...
                    TunnelType.HYSTERIA2 -> connectHysteria2(profile, dnsServer)
                }
    ...
    private fun connectChain(chainId: Long) {
        ...
        connectJob = serviceScope.launch {
            ...
            // Validate the chain
            val validationError = app.tzvpn.domain.model.ChainValidation.validate(profiles)
            ...
            app.tzvpn.util.AppLog.redactSensitive = profiles.any { it.isLocked }
            ...
            if (wakeLock == null) {
                wakeLock = pm.newWakeLock(android.os.PowerManager.PARTIAL_WAKE_LOCK, "TZVPN:VpnWakeLock").apply {
                    setReferenceCounted(false)
                    acquire(WAKELOCK_TIMEOUT_MS)
                }
            }
     ...
     */
    private suspend fun executeChain(profiles: List<app.tzvpn.domain.model.ServerProfile>) {
        ...
        val needsVpnFirst = app.tzvpn.domain.model.ChainValidation.needsVpnFirst(outermost.tunnelType)
    ...
    private suspend fun startChainLayer(
        profile: app.tzvpn.domain.model.ServerProfile,
        layerIndex: Int = 0,
        layerPort: Int,
        layerHost: String,
        prevPort: Int,
        ...
        prevProfile: app.tzvpn.domain.model.ServerProfile? = null,
        isLast: Boolean,
        remoteDns: String,
        ...
        return when (profile.tunnelType) {
            TunnelType.DNSTT, TunnelType.NOIZDNS -> {
                DnsttBridge.setVpnService(this@TZVPNVpnService)
                ...
                if (prevPort > 0 && prevLayerType != null && prevLayerType != TunnelType.SSH) {
                    val prevOutput = app.tzvpn.domain.model.ChainValidation.outputType(prevLayerType)
                    if (prevOutput == app.tzvpn.domain.model.LayerOutput.SOCKS5) {
            ...
            TunnelType.SSH -> {
                ...
                if (prevPort > 0) {
                    ...
                    val output = app.tzvpn.domain.model.ChainValidation.outputType(resolvedPrevType)
                    ...
                    withContext(Dispatchers.IO) {
                        when (output) {
                            app.tzvpn.domain.model.LayerOutput.RAW_TCP -> {
                            ...
                            app.tzvpn.domain.model.LayerOutput.SOCKS5 -> {
            ...
            TunnelType.SNOWFLAKE -> {
                ...
                val upstream = if (prevPort > 0 && prevLayerType != null &&
                    app.tzvpn.domain.model.ChainValidation.outputType(prevLayerType) ==
                    app.tzvpn.domain.model.LayerOutput.SOCKS5
                ) {
    ...
    private fun stopLayer(type: TunnelType) {
        when (type) {
            ...
            TunnelType.VLESS -> {
                if (VlessRealityBridge.isRunning()) VlessRealityBridge.stop()
                VlessBridge.stop()
            }
            TunnelType.HYSTERIA2 -> Hysteria2Bridge.stop()
            else -> {}
        }
    }
     ...
     */
    private suspend fun connectSlipstream(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
    ...
    private suspend fun configureSshInstance(instance: app.tzvpn.tunnel.SshTunnelInstance) {
        val cipher = preferencesDataStore.sshCipher.first()
        val compression = preferencesDataStore.sshCompression.first()
        val isCustom = preferencesDataStore.sshMaxChannelsIsCustom.first()
    ...
    private suspend fun connectSlipstreamSsh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val slipstreamPort = proxyPort + 1
     ...
     */
    private suspend fun connectDnstt(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectSsh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalDnsIp: String? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectNaiveSsh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectNaive(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val naivePort = proxyPort + 1
     ...
     */
    private suspend fun connectDnsttSsh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val dnsttPort = proxyPort + 1
     ...
     */
    private suspend fun connectVaydns(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectVaydnsSsh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String, remoteDnsFallback: String, globalResolverOverride: List<app.tzvpn.domain.model.DnsResolver>? = null) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val vaydnsPort = proxyPort + 1
     ...
     */
    private suspend fun connectDoh(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectSnowflake(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
     ...
     */
    private suspend fun connectSocks5(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String, remoteDns: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
     ...
     */
    private suspend fun connectVless(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        vpnRepository.setCurrentTunnelType(app.tzvpn.domain.model.TunnelType.VLESS)

        // REALITY mode: direct TCP with xtls REALITY handshake via the Go bridge
        if (profile.vlessSecurity == "reality") {
            connectVlessReality(profile, dnsServer)
            return
        }
    ...
    /**
     * Connect using VLESS over REALITY (raw TCP transport, Go-based bridge).
     * Traffic flow:
     * App -> TUN -> hev-socks5-tunnel -> VlessRealityBridge SOCKS5 (proxyPort)
     *   -> REALITY (uTLS fingerprint + auth key) -> VLESS -> Server
     */
    private suspend fun connectVlessReality(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val serverPort = profile.cdnPort

        suspend fun startBridge(): Result<Unit> = withContext(Dispatchers.IO) {
            VlessRealityBridge.start(
                listenPort = proxyPort,
                listenHost = proxyHost,
                serverHost = profile.cdnIp.ifBlank { profile.domain },
                serverPort = serverPort,
                uuid = profile.vlessUuid,
                sni = profile.vlessSni.ifBlank { profile.domain },
                publicKey = profile.vlessRealityPubKey,
                shortId = profile.vlessRealityShortId,
                fingerprint = profile.vlessRealityFp
            )
        }

        if (isProxyOnly) {
            val bridgeResult = startBridge()
            if (bridgeResult.isFailure) {
                connectionManager.onVpnError(bridgeResult.exceptionOrNull()?.message ?: "Failed to start VLESS REALITY bridge")
                stopForeground(STOP_FOREGROUND_REMOVE)
                stopSelf()
                return
            }
            if (!waitForProxyReady(proxyPort, maxAttempts = 30, delayMs = 100)) {
                connectionManager.onVpnError("VLESS REALITY bridge failed to start")
            ...
            vpnRepository.setProxyConnected(profile)
            Log.i(TAG, "Proxy-only mode: VLESS REALITY bridge ready on $proxyHost:$proxyPort")
            finishConnection()
            return
        }
        ...
        Log.d(TAG, "VLESS REALITY tunnel started")
        finishConnection()
    }

    /**
     * Connect using Hysteria2 (QUIC-based proxy with optional Salamander obfs,
     * Go-based bridge). Traffic flow:
     * App -> TUN -> hev-socks5-tunnel -> Hysteria2Bridge SOCKS5 (proxyPort)
     *   -> QUIC/Hysteria2 -> Server
     */
    private suspend fun connectHysteria2(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        vpnRepository.setCurrentTunnelType(app.tzvpn.domain.model.TunnelType.HYSTERIA2)
        ...
        suspend fun startBridge(): Result<Unit> = withContext(Dispatchers.IO) {
            Hysteria2Bridge.start(
                ...
                listenHost = proxyHost,
                serverHost = profile.domain,
                serverPort = profile.cdnPort,
                password = profile.hy2Password,
                sni = profile.hy2Sni.ifBlank { profile.domain },
        ...
        if (isProxyOnly) {
            val bridgeResult = startBridge()
            if (bridgeResult.isFailure) {
                connectionManager.onVpnError(bridgeResult.exceptionOrNull()?.message ?: "Failed to start Hysteria2 bridge")
                stopForeground(STOP_FOREGROUND_REMOVE)
                stopSelf()
                return
            }
            if (!waitForProxyReady(proxyPort, maxAttempts = 30, delayMs = 100)) {
                connectionManager.onVpnError("Hysteria2 bridge failed to start")
            ...
            vpnRepository.setProxyConnected(profile)
            Log.i(TAG, "Proxy-only mode: Hysteria2 bridge ready on $proxyHost:$proxyPort")
            finishConnection()
            return
        }
        ...
        Log.d(TAG, "Hysteria2 tunnel started")
        finishConnection()
    }

    private suspend fun connectSnowflakeSmart(profile: app.tzvpn.domain.model.ServerProfile, dnsServer: String) {
        val proxyPort = preferencesDataStore.proxyListenPort.first()
        val proxyHost = preferencesDataStore.proxyListenAddress.first()
        val torSocksPort = proxyPort + 1
        val snowflakePtPort = proxyPort + 2
        ...
        val transports = listOf(
            ...
            SmartTransport("obfs4", app.tzvpn.presentation.profiles.EditProfileViewModel.DEFAULT_OBFS4_BRIDGES, 60000),
            SmartTransport("Meek Azure", app.tzvpn.presentation.profiles.EditProfileViewModel.DEFAULT_MEEK_BRIDGE, 60000),
    ...
    private suspend fun handleProxyStartupFailure(port: Int) {
        val nativeRunning = when (currentTunnelType) {
            ...
            TunnelType.VLESS -> (if (VlessRealityBridge.isRunning()) VlessRealityBridge.isRunning() else VlessBridge.isRunning())
        }
    ...
    private fun isCurrentProxyHealthy(): Boolean {
        ...
        return when (currentTunnelType) {
            ...
            TunnelType.VLESS -> (if (VlessRealityBridge.isRunning()) VlessRealityBridge.isClientHealthy() else VlessBridge.isRunning() && VlessBridge.isClientHealthy())
            TunnelType.HYSTERIA2 -> Hysteria2Bridge.isRunning() && Hysteria2Bridge.isClientHealthy()
        }
    }
    ...
    private suspend fun cleanupConnection() {
        app.tzvpn.util.AppLog.redactSensitive = false
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        ...
        if (currentProfileId != -1L) {
            ...
            val restartIntent = Intent(this, TZVPNVpnService::class.java).apply {
