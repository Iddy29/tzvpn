...
object VlessRealityBridge {
    ...
    fun start(
        ...
        com.vpntz.app.util.AppLog.i(TAG, "Starting VLESS+REALITY bridge on $listenHost:$listenPort " +
        ...
        return try {
            ...
            client = c
            running.set(true)
            com.vpntz.app.util.AppLog.i(TAG, "VLESS+REALITY bridge started")
            Result.success(Unit)
        } catch (e: Exception) {
            com.vpntz.app.util.AppLog.e(TAG, "Failed to start VLESS+REALITY bridge: ${e.message}", e)
            stop()
            Result.failure(e)
        }
    }
