...
/**
 * Wire protocol numbers (IANA). Kept as the single source of truth for the
 * byte-level network kit so the TUN engine never hardcodes these again.
 */
object Protocol {
    const val TCP = 6
    const val UDP = 17
...
/**
 * Parsed TCP header. [payload] holds the segment bytes that follow the TCP
 * header (excluding the header itself, including any options).
 */
...
/**
 * A parsed IP packet (IPv4 or IPv6). [tcpHeader] is non-null only when the
 * packet is a sufficiently-sized TCP segment. [rawData] is the original bytes
 * supplied to [IpPacketParser.parse].
 */
data class IpPacket(
    val version: Int,
    val protocol: Int,
...
/**
 * Flow key (5-tuple) used for NAT lookup; see [reverse] for the reply-bearing
 * key.
 */
...
/**
 * Deterministic parser for IP datagrams (RFC 791 IPv4, RFC 8200 IPv6) with a
 * TCP header decoder (RFC 793). Pure JVM: it reads only from a [ByteArray] and
 * returns null for any malformed or truncated input rather than throwing.
 */
object IpPacketParser {

    /**
     * Parse an IP packet from raw bytes. Returns null for empty, malformed, or
     * non-IPv4/IPv6 data.
     */
    ...
    /**
     * Extract the connection key from a parsed TCP packet, or null for
     * non-TCP payloads.
     */
