...
const releaseBaseURL = "https://github.com/vpntz/vpn-tz/tzgate/releases"
...
const (
	...
	repoAPI            = "https://api.github.com/repos/anonvector/tzgate/releases"
)

// DownloadBase returns the base URL for tzgate binary downloads.
...
func CheckUpdate() (newVersion string, downloadURL string, err error) {
	...
	// Find matching asset
	target := fmt.Sprintf("tzgate-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func Download(url string) (string, error) {
	tmp, err := os.CreateTemp("", "tzgate-update-*")
	if err != nil {
		return "", err
	}
	defer tmp.Close()
