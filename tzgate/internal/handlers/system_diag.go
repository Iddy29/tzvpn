...
import (
	...
	"github.com/vpntz/vpn-tz/tzgate/internal/version"
...
func handleSystemDiag(ctx *actions.Context) error {
	...
	check("System user (tzgate)", err == nil, boolStr(err == nil, "exists", "missing — run install"))
	...
	for _, t := range cfg.Tunnels {
		// Direct transports use system services (sshd, microsocks), not tzgate-managed ones
		...
		case config.TransportSlipstream:
			if t.TzKitonga != nil {
				checkFileExists(check, tag, "certificate", t.TzKitonga.Cert)
				checkFileExists(check, tag, "key", t.TzKitonga.Key)
			}
		}
	...
	// DNS router
	if hasDNSTunnels {
		routerExists := service.Exists("tzgate-dnsrouter")
		if routerExists {
			...
			if !routerOK {
				showRecentLogs(out, "tzgate-dnsrouter")
			}

			enabled := isServiceEnabled("tzgate-dnsrouter")
	...
	// SOCKS5 proxy
	socksExists := service.Exists("tzgate-socks5")
	if socksExists {
		status, _ := service.Status("tzgate-socks5")
		socksOK := status == "active"
		check("SOCKS5 proxy service", socksOK,
			boolStr(socksOK, "active", status))
		if !socksOK {
			showRecentLogs(out, "tzgate-socks5")
		}
	...
	if cfg.Warp.Enabled {
		...
		warpExists := service.Exists("tzgate-warp")
		if warpExists {
			status, _ := service.Status("tzgate-warp")
			warpOK := status == "active"
			check("WARP service", warpOK,
				boolStr(warpOK, "active", status))
			if !warpOK {
				showRecentLogs(out, "tzgate-warp")
			}
		} else {
		...
		if len(expectedUIDs) == 0 {
			...
			out.Info("  Hint: run 'tzgate restart' or re-enable WARP to fix missing rules")
		}

		// SOCKS5 service user
		if service.Exists("tzgate-socks5") {
			actualUser := service.GetUser("tzgate-socks5")
			userOK := actualUser == warp.SocksUser
			check("SOCKS5 service user", userOK,
				boolStr(userOK,
					fmt.Sprintf("%s (correct)", actualUser),
					fmt.Sprintf("%s — should be %s; run 'tzgate restart' to fix", actualUser, warp.SocksUser)))
		}
	...
	// ── Orphaned Services ───────────────────────────────────
	allSvc := service.ListTzgateServices()
	knownSvc := map[string]bool{
		"tzgate-dnsrouter": true,
		"tzgate-socks5":    true,
		"tzgate-warp":      true,
		"tzgate-iptables":  true,
	}
	for _, t := range cfg.Tunnels {
...
func isResolvedStubDisabled() bool {
	data, err := os.ReadFile("/etc/systemd/resolved.conf.d/tzgate-no-stub.conf")
	if err != nil {
		return false
	}
...
func collectExpectedWarpUIDs(cfg *config.Config) map[string]int {
	...
	// tzgate-naive (Caddy) is intentionally NOT WARP-routed — see
