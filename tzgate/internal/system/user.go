...
// EnsureUser creates the tzgate system user and groups if they don't exist.
...
// RemoveUser removes the tzgate system user and groups.
...
// EnsureDir creates a directory owned by the tzgate user.
...
// AddSSHUser creates a system user in the tzgate-ssh group.
...
// ListSSHUsers returns all users in the tzgate-ssh group.
...
# TzGate global SSH settings
MaxStartups 100:30:300
MaxSessions 100
TCPKeepAlive no
`
	const matchBlock = `
# TzGate SSH tunnel users
Match Group tzgate-ssh
...
`
	...
	if !strings.Contains(content, "# TzGate global SSH settings") {
		// Insert before the first Match block or TzGate block, or append
		if idx := strings.Index(content, "# TzGate SSH tunnel users"); idx >= 0 {
	...
	if strings.Contains(content, "Match Group tzgate-ssh") {
