...
const (
	iptablesRulesPath = "/etc/iptables/tzgate-rules.v4"
	iptablesUnitPath  = "/etc/systemd/system/tzgate-iptables.service"
)
...
Description=Restore tzgate iptables rules
...
func iptablesPersist() error {
	...
	if string(existing) != unit {
		...
		if err := run("systemctl", "enable", "tzgate-iptables.service"); err != nil {
			return err
		}
	}
	return nil
...
// so that tzgate's DNS router can bind to it.
func DisableResolvedStub() error {
	...
	conf := "[Resolve]\nDNSStubListener=no\n"
	if err := os.WriteFile("/etc/systemd/resolved.conf.d/tzgate-no-stub.conf", []byte(conf), 0644); err != nil {
