...
const (
	WarpDir     = "/etc/tzgate/warp"
	WarpConf    = "/etc/tzgate/warp/wg0.conf"
	ProfileFile = "/etc/tzgate/warp/wgcf-profile.conf" // legacy wgcf profile
	ServiceName = "tzgate-warp"
	RouteTable  = 200
...
func Setup(cfg *config.Config, log func(string)) error {
	...
	// upgrading via `tzgate install` get the asymmetric-routing fix
	// without a restart.
	if IsRunning() {
...
func collectUserUIDs(cfg *config.Config) []int {
	...
	// NaiveProxy's user (tzgate-naive) is intentionally NOT routed
	// through WARP. Caddy is just an HTTPS-wrapped TCP forwarder — actual
	// proxied traffic terminates on 127.0.0.1:1080 (tzgate-socks) or
...
func ensureSocksUser() error {
	...
	// Ensure the tzgate group exists
	_ = tryRun("groupadd", "--system", config.SystemGroup)
...
Description=TzGate WARP (Cloudflare WireGuard)
...
// resolvedDropIn is the systemd-resolved drop-in tzgate installs so that
// WARP-routed services can resolve DNS. Removed on Uninstall.
const resolvedDropIn = "/etc/systemd/resolved.conf.d/tzgate-warp-dns.conf"
...
// any WARP-routed tzgate service (NaiveProxy, per-user SOCKS egress, etc.)
