# TzGate — Complete Operator Guide

> The official server for the [VPN-TZ](https://github.com/anonvector/VPN-TZ) Android app.
> Source: https://github.com/vpntz/vpn-tz/tzgate
> VPN-TZ channel: [@VPN-TZ_app](https://t.me/VPN-TZ_app)
...
10. Sharing configs with clients (`vpntz://` URIs)
...
TzGate is a one-binary tunnel manager for Linux servers. You run it on your VPS, and it sets up the server side of every protocol VPN-TZ supports — DNSTT, NoizDNS, VayDNS, tz-kitonga, NaiveProxy, and StunTLS — with `systemd` services, a DNS router, user management, a live dashboard, and `vpntz://` URI generation for one-tap import in the VPN-TZ app.
...
You need one domain (e.g., `example.com`) that you control. Add these records before installing TzGate:
...
```bash
curl -fsSL https://raw.githubusercontent.com/anonvector/tzgate/main/install.sh | sudo bash
```

The script:
- Detects your distro.
- Installs system packages it needs.
- Downloads the TzGate binary plus `dnstt-server`, `slipstream-server`, `vaydns-server`, and `caddy-naive`.
- Installs systemd services (`tzgate-dnsrouter`, `tzgate-socks5`).
- Sets up the firewall rules where applicable.

**Offline install** (when your VPS can't reach GitHub directly):

1. On your laptop, download from https://github.com/vpntz/vpn-tz/tzgate/releases.
2. SCP the binaries to the server: `scp tzgate-* user@server:/tmp/tzgate/`
3. On the server: `sudo tzgate install --bin-dir /tmp/tzgate`
...
TzGate has two interfaces:

**Interactive TUI** — `sudo tzgate` with no arguments. A friendly menu that walks you through tunnels, users, stats, diagnostics, etc. Best for first-time setup and exploration.

**CLI subcommands** — for scripting and automation. Every interactive option has a CLI equivalent:

```
tzgate install                Install dependencies, set up server
tzgate uninstall              Remove everything
tzgate update                 Self-update + restart services
tzgate restart                Restart all services
tzgate users                  Add / list / remove users (interactive)
tzgate users add              Add one user
tzgate users bulk_add         Add many users with random passwords
tzgate users list             List users + per-tunnel configs
tzgate stats                  Live dashboard
tzgate diag                   Run diagnostic checks
tzgate mtu [value]            Set MTU on all DNS tunnels at once

tzgate tunnel add             Add a tunnel
tzgate tunnel edit [tag]      Edit settings (rename, MTU, keys)
tzgate tunnel remove [tag]    Remove one tunnel
tzgate tunnel remove --all    Remove all tunnels
tzgate tunnel start [tag]     Start a tunnel
tzgate tunnel stop [tag]      Stop a tunnel
tzgate tunnel status [tag]    Show status / details
tzgate tunnel share [tag]     Print vpntz:// URI + QR
tzgate tunnel logs [tag]      Show systemd logs

tzgate router status          DNS routing config
tzgate router mode            Switch single ↔ multi mode
tzgate router switch          Change active tunnel (single mode)

tzgate config export          Export full config (JSON)
tzgate config import          Import config back in
```
...
4. Creates a systemd service `tzgate-mytunnel` that starts on boot.

**Confirm it's running:**

```bash
sudo tzgate tunnel status
```
...
TzGate manages six transports. Pick based on the network your users have to escape from.
...
sudo tzgate tunnel add --transport stuntls --tag mytls

# Direct SSH or SOCKS5 (no tunnel transport)
sudo tzgate tunnel add --transport direct-ssh --tag myssh
sudo tzgate tunnel add --transport direct-socks5 --tag mysocks

# External: forward DNS for a domain to a custom UDP port (BYO protocol)
sudo tzgate tunnel add --transport external --tag my-proto \
...
Users in TzGate are **global**. The same `username:password` authenticates against every tunnel, regardless of transport. The protocol is a property of the **tunnel**, not the user.

> ⚠️ **Never hand out the server's `root` (or any sudoer / shell login) account as a VPN credential.** TzGate users are isolated SOCKS/SSH-forwarding accounts with no shell access — losing one only loses tunnel access. A leaked `root` account loses the whole server: your keys, your other users' credentials, and any other service running on the box. Always create dedicated TzGate users with `tzgate users add` (or `bulk_add`) for everyone you give a config to, including yourself.

**Add one user (interactive):**

```bash
sudo tzgate users add
```
...
```bash
sudo tzgate users bulk_add --count=50 --prefix=user
...
```bash
sudo tzgate users list
```

This prints one config block per (user × tunnel) pair, including the `vpntz://` URI for each.

**Remove a user:**

```bash
sudo tzgate users remove
```
...
After creating a tunnel, generate a `vpntz://` URI for one-tap import in the VPN-TZ app:

```bash
sudo tzgate tunnel share mytunnel
```

This prints:
- A `vpntz://...` URI (long base64 string).
- A QR code (in the TUI).

Send the URI to your user. They open VPN-TZ → **+ → Import from URI** → paste → done.
...
You can run as many tunnels as you want. TzGate auto-switches into **multi-mode** when you create a second DNS tunnel — the DNS router on port 53 dispatches queries by domain to the correct tunnel.

Example: DNSTT + VayDNS + NaiveProxy at the same time:

```bash
sudo tzgate tunnel add --transport dnstt   --backend socks --tag dnstt1   --domain t.example.com
sudo tzgate tunnel add --transport vaydns  --backend socks --tag vaydns1  --domain v.example.com
sudo tzgate tunnel add --transport naive   --backend socks --tag naive1   --domain example.com --email you@example.com
```

Each tunnel becomes its own systemd service (`tzgate-dnstt1`, etc.). Manage them like any service:

```bash
sudo tzgate tunnel status
sudo tzgate tunnel start vaydns1
sudo tzgate tunnel stop vaydns1
sudo tzgate tunnel logs naive1
```
...
```bash
sudo tzgate router status      # see current mode
sudo tzgate router mode        # switch between single and multi
sudo tzgate router switch      # change active tunnel (single mode only)
```
...
Enable it from the interactive menu (**WARP** option). TzGate sets up a `wireguard-go` link to WARP and routes user traffic through it.
...
```bash
sudo tzgate tunnel logs mytunnel
```
...
```bash
sudo tzgate tunnel edit --tag mydnstt --new-tag my-tunnel  # rename
sudo tzgate tunnel edit --tag mydnstt --mtu 1232           # change MTU
sudo tzgate tunnel status --tag mydnstt                    # keys, port, MTU
```
...
The update is in-place — your tunnels, users, and keys are untouched. Existing client `vpntz://` URIs keep working.
...
```bash
sudo tzgate config export > tzgate-backup.json
```
...
```bash
sudo tzgate install
sudo tzgate config import < tzgate-backup.json
```

This is also how you migrate to a new VPS without breaking client URIs:
1. Spin up a new VPS.
2. Update your domain's `A` record to the new IP.
3. Install TzGate on the new VPS and import the backup.
4. Existing `vpntz://` URIs keep working — same keys, same domain.

---

## 16. A note on VLESS

The VPN-TZ client supports **VLESS** (over WebSocket / TLS, typically fronted through Cloudflare). **TzGate does not run a VLESS server itself** — VLESS is part of the [Xray / V2Ray](https://github.com/XTLS/Xray-core) ecosystem.

If you want to give your users a VLESS option, you have two paths:

1. **Run Xray separately on the same VPS.** Standard Xray installers (e.g., `Xray-install`, `x-ui`, `3x-ui`) work fine alongside TzGate, since they typically listen on a different port (or on 443 if TzGate isn't using NaiveProxy/StunTLS). Your users build a manual VLESS profile in VPN-TZ using the UUID, domain, and CDN settings from your Xray panel.
2. **Skip VLESS** — TzGate's six transports already cover most censorship environments. NaiveProxy and StunTLS solve the "443 is the only port left" problem.

VLESS support may land in TzGate in the future. Track the channel for updates.
...
| Symptom | Try this |
|---------|----------|
| Tunnel won't start | `sudo tzgate diag` — checks ports, keys, DNS records |
| Client connects but no traffic | `sudo tzgate tunnel logs <tag>` for handshake / auth errors |
| Port 53 already in use | `sudo systemctl disable --now systemd-resolved` (then add `nameserver 1.1.1.1` to `/etc/resolv.conf`) |
| Let's Encrypt fails (NaiveProxy) | A record may not have propagated, or port 80 is blocked. Wait, then `sudo tzgate restart` |
...
| User can't authenticate | `sudo tzgate users list` — confirm credentials present; check tunnel has matching backend |
| Slow DNS tunnel | Lower MTU: `sudo tzgate mtu 1200` |
| Multiple tunnels but only one works | `sudo tzgate router status` — confirm multi-mode is active |
| Need to reset a key | `sudo tzgate tunnel edit --tag <tag>` and regenerate; clients will need a fresh URI |

For deeper inspection, every tunnel is a regular systemd service:

```bash
journalctl -u tzgate-mytunnel -n 200 --no-pager
journalctl -u tzgate-dnsrouter -n 200 --no-pager
journalctl -u tzgate-socks5 -n 200 --no-pager
```
...
| Path | Description |
|------|-------------|
| `/etc/tzgate/config.json` | Main configuration (tunnels, users, routing mode) |
| `/etc/tzgate/tunnels/` | Per-tunnel keys, certs, configs |
| `/usr/local/bin/tzgate` | TzGate binary |
...
| `/etc/caddy/Caddyfile` | Caddy config (managed by TzGate) |

**systemd units:**
- `tzgate-dnsrouter` — port 53 DNS dispatcher
- `tzgate-socks5` — built-in SOCKS5 proxy
- `tzgate-<tag>` — one per tunnel you create
...
Channel: [@VPN-TZ_app](https://t.me/VPN-TZ_app)
TzGate source: https://github.com/vpntz/vpn-tz/tzgate
VPN-TZ source: https://github.com/anonvector/VPN-TZ

---
---

# راهنمای کامل TzGate برای ادمین (فارسی)

> سرور رسمی اپ اندرویدی [VPN-TZ](https://github.com/anonvector/VPN-TZ)
> سورس: https://github.com/vpntz/vpn-tz/tzgate
> کانال VPN-TZ: [@VPN-TZ_app](https://t.me/VPN-TZ_app)
...
TzGate یک مدیر تونل تک‌باینری برای سرورهای لینوکس است. روی VPS اجرا می‌کنید و سمت سرور تمام پروتکل‌هایی که VPN-TZ پشتیبانی می‌کند — DNSTT، NoizDNS، VayDNS، tz-kitonga، NaiveProxy و StunTLS — به همراه سرویس‌های `systemd`، روتر DNS، مدیریت کاربر، داشبورد زنده و تولید لینک `vpntz://` برای ایمپورت یک‌ضربه‌ای در اپ راه‌اندازی می‌کند.
...
TzGate برای شما این کارها را انجام می‌دهد:
...
به یک دامنه نیاز دارید (مثلاً `example.com`). قبل از نصب TzGate این رکوردها را اضافه کنید:
...
- باینری TzGate و `dnstt-server`، `slipstream-server`، `vaydns-server` و `caddy-naive` را دانلود می‌کند.
- سرویس‌های systemd (`tzgate-dnsrouter`، `tzgate-socks5`) را نصب می‌کند.
- در صورت نیاز قواعد فایروال را تنظیم می‌کند.

**نصب آفلاین** (اگر VPS به گیت‌هاب دسترسی ندارد):

۱. روی لپ‌تاپ از https://github.com/vpntz/vpn-tz/tzgate/releases دانلود کنید.
۲. باینری‌ها را با SCP به سرور بفرستید: `scp tzgate-* user@server:/tmp/tzgate/`
۳. روی سرور: `sudo tzgate install --bin-dir /tmp/tzgate`
...
**TUI تعاملی** — `sudo tzgate` بدون پارامتر. منوی دوستانه‌ای که شما را در ساخت تونل، کاربر، آمار و عیب‌یابی راه می‌برد. برای نصب اولیه و کاوش بهترین انتخاب است.

**زیردستورهای CLI** — برای اسکریپت و اتوماسیون. هر گزینه‌ی منو یک معادل CLI دارد:

```
tzgate install                نصب وابستگی‌ها و راه‌اندازی سرور
tzgate uninstall              حذف همه‌چیز
tzgate update                 خود-آپدیت + ری‌استارت سرویس‌ها
tzgate restart                ری‌استارت همه سرویس‌ها
tzgate users                  افزودن / لیست / حذف کاربر (تعاملی)
tzgate users add              افزودن یک کاربر
tzgate users bulk_add         افزودن گروهی با پسورد تصادفی
tzgate users list             لیست کاربران + کانفیگ تونل‌ها
tzgate stats                  داشبورد زنده
tzgate diag                   اجرای بررسی‌های تشخیصی
tzgate mtu [value]            تنظیم MTU روی همه‌ی تونل‌های DNS

tzgate tunnel add             افزودن یک تونل
tzgate tunnel edit [tag]      ویرایش (نام، MTU، کلید)
tzgate tunnel remove [tag]    حذف یک تونل
tzgate tunnel remove --all    حذف همه‌ی تونل‌ها
tzgate tunnel start [tag]     استارت تونل
tzgate tunnel stop [tag]      استاپ تونل
tzgate tunnel status [tag]    وضعیت / جزئیات
tzgate tunnel share [tag]     چاپ vpntz:// URI + QR
tzgate tunnel logs [tag]      دیدن لاگ‌های systemd

tzgate router status          وضعیت روتر DNS
tzgate router mode            تعویض single ↔ multi
tzgate router switch          تغییر تونل فعال (single mode)

tzgate config export          خروجی کامل کانفیگ (JSON)
tzgate config import          ایمپورت دوباره
```
...
۴. سرویس systemd `tzgate-mytunnel` می‌سازد که در بوت اجرا می‌شود.
...
TzGate شش transport را مدیریت می‌کند. بر اساس شبکه‌ای که کاربران از آن می‌خواهند فرار کنند انتخاب کنید.
...
کاربر در TzGate **سراسری** است. همان `username:password` روی همه‌ی تونل‌ها معتبر است، فارغ از transport. پروتکل ویژگی **تونل** است نه کاربر.

> ⚠️ **هیچ‌وقت اکانت `root` سرور (یا هر اکانت sudoer / لاگین shell) را به‌عنوان credential VPN در اختیار کاربران نگذارید.** کاربر TzGate یک اکانت ایزوله‌ی SOCKS/SSH-forwarding است که دسترسی shell ندارد — اگر یکی لو برود، فقط دسترسی به تونل از دست می‌رود. اما لو رفتن `root` یعنی از دست رفتن کل سرور: کلیدها، credential سایر کاربران و هر سرویس دیگری که روی سرور است. همیشه برای هر کسی که کانفیگ می‌دهید (حتی خودتان) با `tzgate users add` (یا `bulk_add`) یک کاربر مخصوص بسازید.
...
برای هر زوج (کاربر × تونل) یک بلوک کانفیگ چاپ می‌شود، شامل URI به فرمت `vpntz://`.
...
پس از ساخت تونل، یک URI به فرمت `vpntz://` بسازید:
...
این چاپ می‌کند:
- یک URI `vpntz://...` (رشته‌ی base64 طولانی).
- یک کد QR (در TUI).

URI را به کاربر بدهید. در VPN-TZ **+ → Import from URI** را می‌زند → پیست → تمام.
...
می‌توانید هر تعداد تونل می‌خواهید اجرا کنید. TzGate وقتی تونل DNS دوم را می‌سازید خودکار به **multi-mode** سوییچ می‌کند — روتر DNS روی پورت ۵۳ کوئری را بر اساس دامنه به تونل درست می‌فرستد.
...
هر تونل سرویس systemd خودش را دارد (`tzgate-dnstt1` و…). مثل هر سرویس مدیریتش کنید:
...
```bash
sudo tzgate router status      # حالت فعلی
sudo tzgate router mode        # تغییر بین single و multi
sudo tzgate router switch      # تغییر تونل فعال (فقط در single mode)
```
...
از منوی تعاملی گزینه‌ی **WARP** را روشن کنید. TzGate یک لینک `wireguard-go` به WARP می‌سازد و ترافیک کاربر را از آن عبور می‌دهد.
...
```bash
sudo tzgate tunnel edit --tag mydnstt --new-tag my-tunnel  # تغییر نام
sudo tzgate tunnel edit --tag mydnstt --mtu 1232           # تغییر MTU
sudo tzgate tunnel status --tag mydnstt                    # کلید، پورت، MTU
```
...
۳. TzGate را روی VPS جدید نصب کنید و بکاپ را ایمپورت کنید.
۴. URIهای موجود `vpntz://` کار می‌کنند — همان کلیدها، همان دامنه.

---

## ۱۶. درباره‌ی VLESS

اپ VPN-TZ از **VLESS** پشتیبانی می‌کند (روی WebSocket / TLS، معمولاً پشت Cloudflare). **اما TzGate خودش سرور VLESS اجرا نمی‌کند** — VLESS بخشی از اکوسیستم [Xray / V2Ray](https://github.com/XTLS/Xray-core) است.

اگر می‌خواهید به کاربران گزینه‌ی VLESS بدهید، دو راه دارید:

۱. **Xray را جدا روی همان VPS اجرا کنید.** نصاب‌های استاندارد Xray (مثل `Xray-install`، `x-ui`، `3x-ui`) در کنار TzGate خوب کار می‌کنند، چون معمولاً روی پورت متفاوتی گوش می‌دهند (یا روی ۴۴۳ اگر TzGate از NaiveProxy/StunTLS استفاده نمی‌کند). کاربر شما در VPN-TZ به‌صورت دستی پروفایل VLESS می‌سازد با UUID، دامنه، و تنظیمات CDN از پنل Xray شما.
۲. **VLESS را رها کنید** — شش transport TzGate برای اکثر محیط‌های سانسوری کافی است. NaiveProxy و StunTLS مشکل «فقط ۴۴۳ باز است» را حل می‌کنند.

ممکن است در آینده پشتیبانی VLESS به TzGate اضافه شود. کانال را برای اپدیت دنبال کنید.
...
| نشانه | راه‌حل |
|-------|------|
| تونل بالا نمی‌آید | `sudo tzgate diag` — پورت‌ها، کلیدها و رکوردهای DNS را بررسی می‌کند |
| کلاینت وصل می‌شود ولی ترافیک نمی‌رود | `sudo tzgate tunnel logs <tag>` برای دیدن خطای handshake / احراز هویت |
...
| Let's Encrypt در NaiveProxy fail می‌شود | احتمالاً رکورد A پراپاگیت نشده یا پورت ۸۰ بسته است. صبر کنید و `sudo tzgate restart` |
...
| کاربر احراز نمی‌شود | `sudo tzgate users list` — وجود credential را بررسی کنید؛ مطمئن شوید تونل backend متناظر دارد |
| تونل DNS کند است | MTU را پایین بیاورید: `sudo tzgate mtu 1200` |
| چند تونل ساخته‌ام ولی فقط یکی کار می‌کند | `sudo tzgate router status` — تأیید کنید multi-mode فعال است |
| نیاز به ریست کلید دارم | `sudo tzgate tunnel edit --tag <tag>` و کلید جدید بسازید؛ کلاینت‌ها به URI جدید نیاز دارند |
...
| مسیر | توضیح |
|------|------|
| `/etc/tzgate/config.json` | کانفیگ اصلی (تونل، کاربر، حالت روتینگ) |
| `/etc/tzgate/tunnels/` | کلید، گواهی و کانفیگ هر تونل |
| `/usr/local/bin/tzgate` | باینری TzGate |
...
| `/etc/caddy/Caddyfile` | کانفیگ Caddy (مدیریت‌شده توسط TzGate) |

**یونیت‌های systemd:**
- `tzgate-dnsrouter` — دیسپچر DNS روی پورت ۵۳
- `tzgate-socks5` — پراکسی SOCKS5 داخلی
- `tzgate-<tag>` — یکی به ازای هر تونل
...
کانال: [@VPN-TZ_app](https://t.me/VPN-TZ_app)
سورس TzGate: https://github.com/vpntz/vpn-tz/tzgate
سورس VPN-TZ: https://github.com/anonvector/VPN-TZ
