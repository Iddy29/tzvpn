...
# ══════════════════════════════════════════
#  VPN-TZ Server Setup — NaiveProxy + SSH
# ══════════════════════════════════════════
...
ensure_vpntz_group() {
    if ! getent group vpntz &>/dev/null; then
        groupadd vpntz
        echo "  Created 'vpntz' group."
    fi
    if ! grep -q "^# BEGIN VPN-TZ" /etc/ssh/sshd_config 2>/dev/null; then
...
SSHEOF
        ...
        echo "  Configured sshd for VPN-TZ tunnel users."
    fi
}
...
print_header() {
    echo ""
    echo "══════════════════════════════════════════"
    echo "  VPN-TZ Server — NaiveProxy + SSH"
    echo "══════════════════════════════════════════"
}
...
do_install() {
    ...
    if [[ "${CREATE_SSH}" == [yY] ]]; then
        ...
        if ! id "${PROXY_USER}" &>/dev/null; then
            useradd -m -s /bin/false -G vpntz "${PROXY_USER}"
        else
            usermod -aG vpntz "${PROXY_USER}"
        fi
...
[Unit]
Description=VPN-TZ Caddy-Naive Proxy Server
...
do_show_config() {
    ...
    echo "══════════════════════════════════════════"
    echo " VPN-TZ Server — Current Configuration"
    echo "══════════════════════════════════════════"
    echo " Status:  ${status}"
    echo " Domain:  ${domain}"
    echo " Email:   ${email}"
    ...
    list_users
    echo "──────────────────────────────────────────"
    echo " VPN-TZ App Profile Settings:"
...
do_uninstall() {
    ...
    echo "This will remove caddy-naive, all VPN-TZ SSH users, and configuration."
    ...
    echo "[3/5] Removing VPN-TZ SSH users..."
    if getent group vpntz &>/dev/null; then
        local members
        members="$(getent group vpntz | cut -d: -f4)" || members=""
        if [[ -n "${members}" ]]; then
        ...
        groupdel vpntz 2>/dev/null || true
        echo "  Removed vpntz group."
    else
        echo "  No vpntz group found."
    fi

    if grep -q "^# BEGIN VPN-TZ" /etc/ssh/sshd_config 2>/dev/null; then
        sed -i '/^# BEGIN VPN-TZ$/,/^# END VPN-TZ$/d' /etc/ssh/sshd_config
        systemctl reload sshd 2>/dev/null || systemctl reload ssh 2>/dev/null || true
...
add_user() {
    ...
    if [[ "${create_ssh}" == [yY] ]]; then
        ensure_vpntz_group
        if ! useradd -m -s /bin/false -G vpntz "${username}" 2>/dev/null; then
...
print_summary() {
    echo ""
    echo "══════════════════════════════════════════"
    echo " VPN-TZ Server Setup Complete"
    echo "══════════════════════════════════════════"
    if [[ "${CREATE_SSH}" == [yY] ]]; then
        echo " Tunnel Type:    NaiveProxy + SSH"
    else
