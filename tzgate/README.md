...
Unified tunnel manager for Linux servers. Manages DNS tunnels (DNSTT, NoizDNS, tz-kitonga, VayDNS) and HTTPS proxies (NaiveProxy) with systemd services, multi-tunnel DNS routing, and user management. Designed for use with the [VPN-TZ](https://github.com/anonvector/VPN-TZ) Android VPN app.

[![oosmetrics](https://api.oosmetrics.com/api/v1/badge/achievement/12767996-2dd6-42ed-89a6-5f786c4662b4.svg)](https://oosmetrics.com/repo/anonvector/tzgate)
...
- **Client sharing**: Generates `vpntz://` URIs for one-tap app import
...
```bash
git clone https://github.com/vpntz/vpn-tz/tzgate.git
cd tzgate
make build
sudo ./tzgate install
```
...
Download the binaries you need from the [latest release](https://github.com/vpntz/vpn-tz/tzgate/releases):

```bash
# On your local machine — download binaries
mkdir tzgate-bundle && cd tzgate-bundle
curl -LO https://github.com/vpntz/vpn-tz/tzgate/releases/latest/download/tzgate-linux-amd64
curl -LO https://github.com/vpntz/vpn-tz/tzgate/releases/latest/download/dnstt-server-linux-amd64
curl -LO https://github.com/vpntz/vpn-tz/tzgate/releases/latest/download/slipstream-server-linux-amd64
curl -LO https://github.com/vpntz/vpn-tz/tzgate/releases/latest/download/caddy-naive-linux-amd64

# SCP to server
scp * user@server:/tmp/tzgate/

# On the server
chmod +x /tmp/tzgate/*
sudo cp /tmp/tzgate/tzgate-linux-amd64 /usr/local/bin/tzgate
sudo tzgate install --bin-dir /tmp/tzgate
```
...
```
tzgate                        # Interactive TUI menu
tzgate install                # Install dependencies and configure server
tzgate uninstall              # Remove all services, configs, and binaries
tzgate update                 # Self-update and restart all services
tzgate restart                # Restart all services (DNS router, tunnels, SOCKS)
tzgate users                  # Manage SSH/SOCKS users and view configs
tzgate users add              # Add a single user
tzgate users bulk_add         # Add multiple users in one batch (random creds, up to 500)
tzgate users remove           # Remove a user
tzgate users list             # List users and their per-tunnel configs
tzgate stats                  # Live dashboard (CPU, RAM, traffic, connections, tunnels)
tzgate diag                   # Run diagnostics (services, ports, keys, DNS, boot status)
tzgate mtu [value]            # Set MTU for all DNSTT/NoizDNS/VayDNS tunnels at once

# Tunnel management
tzgate tunnel add             # Add tunnel(s) — supports multi-select and "both" backend
tzgate tunnel edit [tag]      # Edit tunnel settings (tag, MTU, keys)
tzgate tunnel remove [tag]    # Remove a tunnel
tzgate tunnel remove --all    # Remove all tunnels at once
tzgate tunnel start [tag]     # Start a tunnel
tzgate tunnel stop [tag]      # Stop a tunnel
tzgate tunnel status          # Show all tunnel statuses
tzgate tunnel status [tag]    # Show tunnel details (keys, MTU, port)
tzgate tunnel share [tag]     # Generate vpntz:// URI for clients
tzgate tunnel logs [tag]      # View tunnel logs

# DNS routing
tzgate router status          # Show DNS routing config
tzgate router mode            # Switch between single/multi mode
tzgate router switch          # Change active tunnel (single mode)

# Configuration
tzgate config export          # Export configuration
tzgate config import          # Import configuration

# Internal (used by systemd services)
tzgate dnsrouter serve        # Start DNS router
tzgate socks serve            # Start built-in SOCKS5 proxy
tzgate stuntls serve          # Start StunTLS proxy
```

### Non-Interactive Examples

All commands support flags for scripting and automation. If any required flag is omitted, tzgate falls back to an interactive prompt.
...
# Rename a tunnel
sudo tzgate tunnel edit --tag mydnstt --new-tag my-tunnel

# Change MTU on a DNSTT tunnel
sudo tzgate tunnel edit --tag mydnstt --mtu 1232
...
sudo tzgate tunnel edit --tag myvaydns \
  --mtu 1232 \
  --record-type txt \
  --idle-timeout 10s \
  --keep-alive 2s \
...
sudo tzgate tunnel status --tag mydnstt

# Share tunnel config as vpntz:// URI
sudo tzgate tunnel share mydnstt
...
└──────────────────────────────────────────────────────────────┘
  ...
  systemd: tzgate-dnsrouter, tzgate-socks5, tzgate-{tag}
```
...
This outputs a `vpntz://` URI that can be scanned or imported into the VPN-TZ Android app. For DNSTT tunnels, you'll be asked to choose between a DNSTT or NoizDNS client profile — both connect to the same server, but NoizDNS profiles enable DPI evasion on the client side.

### User Model

Users are **global**, not scoped to specific tunnels or transports. `tzgate users add` only asks for a username and password — the protocol is a property of the tunnel, chosen at `tunnel add` time. Every user can authenticate against every tunnel using the same credentials, and `tzgate users list` prints one config block per (user × tunnel) pair. The client picks which tunnel to use by importing the matching `vpntz://` URI.

## File Locations

| Path | Description |
|------|-------------|
| `/etc/tzgate/config.json` | Main configuration |
| `/etc/tzgate/tunnels/` | Per-tunnel keys, certs, and configs |
| `/usr/local/bin/tzgate` | TzGate binary (includes built-in SOCKS5 proxy) |
...
hysteria2:// link (plus QR if qrencode is installed) for the VPN-TZ app.
