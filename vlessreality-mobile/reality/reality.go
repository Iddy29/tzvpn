...
// (https://github.com/XTLS/Xray-core) so the TZVPN CLI can speak VLESS over
...
// Config holds the REALITY client parameters for one server.
type Config struct {
...
// conn is a utls connection with REALITY handshake + verification.
type conn struct {
	*utls.UConn
	config     *Config
	serverName string
...
// Dial connects to addr over TCP and performs the REALITY handshake.
...
// client performs the REALITY handshake over an established TCP conn.
func client(raw net.Conn, cfg *Config) (net.Conn, error) {
	...
	u := &conn{config: cfg, serverName: cfg.ServerName}
...
func (u *conn) verifyPeerCertificate(rawCerts [][]byte, _ [][]*x509.Certificate) error {
	if len(rawCerts) == 0 {
