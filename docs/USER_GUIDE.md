# TZVPN — Complete User Guide

> Official repository: [Iddy29/tzvpn](https://github.com/Iddy29/tzvpn)
> Source code: https://github.com/Iddy29/tzvpn
> TZVPN is **NOT** on Google Play, the App Store, or any other marketplace. Only download it from the official channel or GitHub.

---

## Table of Contents

1. What TZVPN is — and why it exists
...
TZVPN is a free, **source-available** anti-censorship tool. The main client is an Android app, with a matching command-line client for macOS, Linux, and Windows. It tunnels your internet traffic through different protocols — DNS, QUIC, SSH, HTTPS, VLESS, or Tor — so you can reach the open internet even on networks that block, throttle, or deep-packet-inspect normal traffic.

Unlike a typical VPN that ships with a single protocol, TZVPN offers many **transports** so you can pick the one that survives on whatever network you're stuck on:
...
TZVPN ships in two editions — the difference is the size of the APK and which protocols are bundled.
...
1. Open the official GitHub repository [Iddy29/tzvpn](https://github.com/Iddy29/tzvpn) or the GitHub Releases page.
...
5. Open TZVPN. On first launch, Android will ask for **VPN permission** — tap **OK**.
6. Recommended one-time setup: **Settings → Battery optimization → Don't optimize TZVPN**, so Android doesn't kill it in the background.

> ⚠️ If you find TZVPN on Google Play, App Store, or any other store, it is **not** ours. Don't install it.
...
### A) Paste a `tzvpn://` link (easiest)

1. Open TZVPN → tap the **+** button at the bottom right.
2. Choose **Import from URI**.
3. Paste the `tzvpn://...` link.
...
- Pull down the notification shade → tap the **TZVPN Quick Settings tile** (you may need to add it from the tile picker the first time).
...
- **Transport** — WebSocket only (raw TCP isn't supported in the app — VLESS in TZVPN is positioned as a CDN-fronted tunnel)
- **WS Path** — e.g., `/`, `/vless`
...
TZVPN's SOCKS5 mode is a **passthrough** — it just opens a local listener on your phone and forwards every downstream request to the remote SOCKS5 proxy. There is **no upfront end-to-end probe** of the upstream. The "Connected" status only confirms two things:
...
- **Share a single profile**: open it → menu → **Export** → choose `tzvpn://` URI or JSON.
...
TZVPN exports use three URI schemes. They are **not** equivalent — the security model is very different.

| Scheme | What it is | Encryption key |
|---|---|---|
| `tzvpn://` | Plain base64-encoded profile | None — anyone can decode it |
| `tzvpn-enc://` | Single **locked** profile (single-recipient share) | App-baked key from TZVPN's native library |
| `tzvpn-bundle-enc://` | Multi-profile **password-encrypted** backup bundle | Derived from your password |

**`tzvpn://`** — readable by anyone who can run base64. Use only for trusted recipients.

**`tzvpn-enc://`** — produced when you export a single profile with a lock (password, optional expiration date, optional device-binding, hidden resolvers, no-resharing). The payload is AES-256-GCM encrypted, **but the AES key is baked into the TZVPN app itself**, not derived from your password. The password you set is checked by the recipient's app — it controls **how** the recipient can use the profile (use only, no editing, expires on date X, bound to one device, resolvers hidden). It does **not** keep your config secret from anyone with the TZVPN APK. Treat it as a **sharing-control** mechanism, not real confidentiality.

**`tzvpn-bundle-enc://`** — produced by **Export All (Encrypted)**. This one *is* real password-derived encryption (your password is the key material). Suitable for off-device backups.

### ⚠️ Even encrypted configs are unsafe to post publicly

Encrypting the config bytes does **not** protect the **identity** of your server, domain, or CDN host. The moment a `tzvpn-enc://` link reaches a public channel:

- Anyone with the TZVPN APK can decrypt it and read the domain, IP, SNI, CDN host, and credentials.
...
> ⚠️ A `tzvpn://` link contains your credentials. Never post yours in a public group.
> ⚠️ A `tzvpn-enc://` link is **not** safe to post publicly either — the TZVPN app can decrypt it. The lock controls usage, not secrecy. The encryption protects the *credentials* on the wire; it does **not** protect the *domain identity* once the link is in the open.

---

## 14. Donations

Development is unpaid. If TZVPN helps you, donation addresses (BEP-20 / ERC-20 / Arbitrum, Monero, etc.) are kept up to date in the project README on GitHub: <https://github.com/Iddy29/tzvpn#donations>

---

## 15. Stay safe

- Only download TZVPN from the **official Telegram channel** or **GitHub**.
...
# راهنمای کامل TZVPN (فارسی)

> مخزن رسمی: https://github.com/Iddy29/tzvpn
> سورس‌کد: https://github.com/Iddy29/tzvpn
> TZVPN در گوگل‌پلی، اپ‌استور یا هیچ مارکت دیگری نیست. فقط از کانال رسمی یا گیت‌هاب دانلود کنید.

---

## فهرست

- ۱. TZVPN چیست و چرا ساخته شده
...
TZVPN یک ابزار رایگان و **source-available** (سورس‌کدش قابل مطالعه است، اما اجازه‌ی توزیع مجدد ندارد) برای دور زدن سانسور و بازرسی شبکه است. نسخه‌ی اصلی برای اندروید است و یک کلاینت CLI هم برای مک، لینوکس و ویندوز ارائه می‌شود. TZVPN ترافیک اینترنت شما را از داخل پروتکل‌های متفاوتی مثل DNS، QUIC، SSH، HTTPS، VLESS یا Tor عبور می‌دهد تا حتی در شبکه‌هایی که اینترنت در آن‌ها فیلتر، محدود یا با DPI بازرسی می‌شود، بتوانید آزادانه به وب دسترسی پیدا کنید.

برخلاف یک VPN معمولی که تنها یک پروتکل دارد، TZVPN چند **حامل (transport)** مختلف ارائه می‌کند تا متناسب با شرایط شبکه‌ای که در آن گیر افتاده‌اید بتوانید بهترین مسیر را انتخاب کنید:
...
سورس‌کد TZVPN به‌صورت کامل روی گیت‌هاب در دسترس است (تحت لایسنس source-available — قابل مطالعه و مشارکت، اما بدون اجازه‌ی توزیع مجدد یا انتشار در استورها) و در هیچ مارکت اپ (گوگل‌پلی، اپ‌استور و…) منتشر نمی‌شود؛ تنها منبع رسمی، مخزن [Iddy29/tzvpn](https://github.com/Iddy29/tzvpn) و مخزن گیت‌هاب پروژه است.
...
TZVPN در دو نسخه عرضه می‌شود — تفاوت در حجم APK و پروتکل‌های موجود است.
...
۱. کانال رسمی [Iddy29/tzvpn](https://github.com/Iddy29/tzvpn) یا صفحه‌ی Releases در گیت‌هاب را باز کنید.
...
۶. تنظیم پیشنهادی یک‌باره: **تنظیمات → بهینه‌سازی باتری → TZVPN را بهینه نکن** تا اندروید آن را در پس‌زمینه نکشد.

> ⚠️ اگر TZVPN را در گوگل‌پلی، اپ‌استور یا هر مارکت دیگری دیدید، **مال ما نیست**؛ نصب نکنید.
...
### الف) چسباندن لینک `tzvpn://` (ساده‌ترین)

۱. TZVPN را باز کنید → دکمه‌ی **+** پایین سمت راست.
۲. **Import from URI** را انتخاب کنید.
۳. لینک `tzvpn://...` را پیست کنید.
...
- نوار اعلان را پایین بکشید → **کاشی Quick Settings** TZVPN را بزنید (اولین بار باید از پنل کاشی‌ها اضافه‌اش کنید).
...
- **Transport** — فقط WebSocket (TCP خام در اپ پشتیبانی نمی‌شود — VLESS در TZVPN به‌عنوان تونل پشت CDN در نظر گرفته شده)
- **WS Path** — مثلاً `/`، `/vless`
...
حالت SOCKS5 در TZVPN یک **passthrough** ساده است — فقط یک listener محلی روی گوشی باز می‌کند و هر درخواست را به پراکسی SOCKS5 ریموت زنجیر می‌کند. **هیچ تست end-to-end اولیه‌ای انجام نمی‌شود.** وضعیت «Connected» فقط دو چیز را تأیید می‌کند:
...
- **اشتراک یک پروفایل**: روی پروفایل بزنید → منو → **Export** → URI `tzvpn://` یا JSON.
...
TZVPN سه نوع URI برای خروجی دارد. این‌ها **هم‌ارز نیستند** — مدل امنیتی هرکدام فرق می‌کند.

| فرمت | چیست | کلید رمزنگاری |
|---|---|---|
| `tzvpn://` | پروفایل ساده‌ی base64 | ندارد — هرکسی می‌تواند بخواند |
| `tzvpn-enc://` | پروفایل تکیِ **قفل‌شده** (مخصوص گیرنده‌ی خاص) | کلید درون‌برنامه‌ای TZVPN |
| `tzvpn-bundle-enc://` | بسته‌ی چندپروفایلی، رمز شده با رمز عبور | از رمز عبور خود شما مشتق می‌شود |

**`tzvpn://`** — هرکسی با base64 می‌تواند بخواند. فقط برای گیرنده‌ی مورد اعتماد.

**`tzvpn-enc://`** — وقتی یک پروفایل تکی را با قفل خروجی می‌گیرید (رمز، تاریخ انقضا اختیاری، قید به یک دستگاه، مخفی‌سازی resolverها، عدم اجازه‌ی بازنشر). محتوا با AES-256-GCM رمز می‌شود، **اما کلید AES درون اپ TZVPN جاسازی شده** و از رمز شما مشتق نمی‌شود. رمزی که می‌گذارید توسط اپ گیرنده بررسی می‌شود و فقط **نحوه‌ی استفاده** را کنترل می‌کند (فقط استفاده، عدم ویرایش، انقضا، قید به دستگاه، مخفی‌بودن resolverها). محرمانگی کانفیگ در برابر کسی که اپ را نصب دارد تأمین نمی‌شود. این را یک ابزار **کنترل اشتراک‌گذاری** بدانید، نه محرمانگی واقعی.

**`tzvpn-bundle-enc://`** — توسط **Export All (Encrypted)** ساخته می‌شود. این یکی واقعاً با رمز عبور خودتان رمزنگاری می‌شود و برای پشتیبان‌گیری مناسب است.

### ⚠️ حتی کانفیگ رمزشده هم نباید در فضای عمومی منتشر شود

رمزنگاریِ بایت‌های کانفیگ، **هویت** سرور، دامنه یا CDN شما را پنهان نمی‌کند. به محض اینکه یک لینک `tzvpn-enc://` در کانال عمومی منتشر شود:

- هرکسی که APK TZVPN را داشته باشد می‌تواند آن را باز کند و دامنه، IP، SNI، CDN host و credential را ببیند.
...
> ⚠️ لینک `tzvpn://` حاوی اطلاعات حساس شماست؛ هرگز آن را در گروه عمومی نگذارید.
>
> ⚠️ لینک `tzvpn-enc://` هم **برای انتشار عمومی امن نیست**: هر کسی که اپ TZVPN را داشته باشد می‌تواند آن را رمزگشایی کند. قفل، فقط *نحوه‌ی استفاده* را محدود می‌کند و **محرمانگی محتوا را تضمین نمی‌کند**. رمزنگاری از credentialها در حین انتقال محافظت می‌کند، اما به محض اینکه لینک علنی شد، **هویت دامنه و سرور شما لو می‌رود** و رمزنگاری نمی‌تواند جلویش را بگیرد.

---

## ۱۴. حمایت مالی

توسعه‌ی TZVPN داوطلبانه و بدون درآمد است. اگر این اپ به شما کمک کرده، آدرس‌های دونیت (BEP-20 / ERC-20 / Arbitrum، Monero و…) همیشه به‌روز در README پروژه روی گیت‌هاب نگه‌داری می‌شوند: <https://github.com/Iddy29/tzvpn#donations>
