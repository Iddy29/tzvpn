# VPN-TZ — Complete User Guide

> Official repository: [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz)
> Source code: https://github.com/vpntz/vpn-tz
> VPN-TZ is **NOT** on Google Play, the App Store, or any other marketplace. Only download it from the official channel or GitHub.

---

## Table of Contents

1. What VPN-TZ is — and why it exists
2. Supported tunnel types
3. Editions: Full vs. Lite
4. Installing the APK
5. You need a server (use TzGate)
...
VPN-TZ is a free, **source-available** anti-censorship tool. The main client is an Android app, with a matching command-line client for macOS, Linux, and Windows. It tunnels your internet traffic through different protocols — DNS, QUIC, SSH, HTTPS, VLESS, or Tor — so you can reach the open internet even on networks that block, throttle, or deep-packet-inspect normal traffic.

Unlike a typical VPN that ships with a single protocol, VPN-TZ offers many **transports** so you can pick the one that survives on whatever network you're stuck on:
...
VPN-TZ sits in the same family as projects like [Tor](https://torproject.org), [Psiphon](https://psiphon.ca), and [Outline VPN](https://getoutline.org), but with one critical difference: VPN-TZ is **pure client software**. There is no central network or company-run infrastructure — you either run your own server (a few minutes with [TzGate](https://github.com/vpntz/vpn-tz/tzgate)) or get a config from someone you trust. That model has three real advantages:
...
The full source code lives on GitHub under a **source-available** license — readable, studyable, and open to contributions, but not licensed for redistribution or republishing on app stores. VPN-TZ is **not** on Google Play, the App Store, or any other marketplace; the only official sources are the project's GitHub repository.
...
VPN-TZ ships in two editions — the difference is the size of the APK and which protocols are bundled.
...
1. Open the official GitHub repository [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz) or the GitHub Releases page.
...
5. Open VPN-TZ. On first launch, Android will ask for **VPN permission** — tap **OK**.
6. Recommended one-time setup: **Settings → Battery optimization → Don't optimize VPN-TZ**, so Android doesn't kill it in the background.

> ⚠️ If you find VPN-TZ on Google Play, App Store, or any other store, it is **not** ours. Don't install it.

---

## 5. You need a server — use TzGate

VPN-TZ is a *client*. To use it you must connect to a **server** running a compatible tunnel.

The official, supported way to run a server is **TzGate** — a one-command Linux installer that sets up every protocol VPN-TZ supports.

> https://github.com/vpntz/vpn-tz/tzgate

A separate operator guide for TzGate is available alongside this document. In short: get a $5/month VPS, point a domain at it, run a single install command, create a user, and TzGate prints a `vpntz://` link you paste straight into the app.

If you don't run your own server, you can:
- Get a `vpntz://` link from a friend who runs TzGate, or
- Use a public test link sometimes shared in [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz).
...
### A) Paste a `vpntz://` link (easiest)

1. Open VPN-TZ → tap the **+** button at the bottom right.
2. Choose **Import from URI**.
3. Paste the `vpntz://...` link.
...
- Pull down the notification shade → tap the **VPN-TZ Quick Settings tile** (you may need to add it from the tile picker the first time).
...
#### Prism
*"Server-verified scan: only resolvers that cryptographically prove they reach your specific server are shown. Requires TzGate installed on your server."*

The strongest mode. Sends HMAC-authenticated probes that only a real TzGate server can sign correctly, so a resolver that "works" but actually delivers traffic to an attacker's middlebox will fail the check. Each resolver is probed N times; resolvers that get ≥ *threshold* signed responses pass. Use this when you suspect transparent-proxy interception or want defense against tunnel hijacking.

Only available when your selected profile has a valid public key and your server runs [TzGate](https://github.com/vpntz/vpn-tz/tzgate).
...
- **Transport** — WebSocket only (raw TCP isn't supported in the app — VLESS in VPN-TZ is positioned as a CDN-fronted tunnel)
- **WS Path** — e.g., `/`, `/vless`
...
VPN-TZ's SOCKS5 mode is a **passthrough** — it just opens a local listener on your phone and forwards every downstream request to the remote SOCKS5 proxy. There is **no upfront end-to-end probe** of the upstream. The "Connected" status only confirms two things:
...
| Phone kills VPN-TZ in background | Settings → **Battery optimization** → don't optimize VPN-TZ |
...
- **Share a single profile**: open it → menu → **Export** → choose `vpntz://` URI or JSON.
...
VPN-TZ exports use three URI schemes. They are **not** equivalent — the security model is very different.

| Scheme | What it is | Encryption key |
|---|---|---|
| `vpntz://` | Plain base64-encoded profile | None — anyone can decode it |
| `vpntz-enc://` | Single **locked** profile (single-recipient share) | App-baked key from VPN-TZ's native library |
| `vpntz-bundle-enc://` | Multi-profile **password-encrypted** backup bundle | Derived from your password |

**`vpntz://`** — readable by anyone who can run base64. Use only for trusted recipients.

**`vpntz-enc://`** — produced when you export a single profile with a lock (password, optional expiration date, optional device-binding, hidden resolvers, no-resharing). The payload is AES-256-GCM encrypted, **but the AES key is baked into the VPN-TZ app itself**, not derived from your password. The password you set is checked by the recipient's app — it controls **how** the recipient can use the profile (use only, no editing, expires on date X, bound to one device, resolvers hidden). It does **not** keep your config secret from anyone with the VPN-TZ APK. Treat it as a **sharing-control** mechanism, not real confidentiality.

**`vpntz-bundle-enc://`** — produced by **Export All (Encrypted)**. This one *is* real password-derived encryption (your password is the key material). Suitable for off-device backups.

### ⚠️ Even encrypted configs are unsafe to post publicly

Encrypting the config bytes does **not** protect the **identity** of your server, domain, or CDN host. The moment a `vpntz-enc://` link reaches a public channel:

- Anyone with the VPN-TZ APK can decrypt it and read the domain, IP, SNI, CDN host, and credentials.
...
- For wider distribution, host on a **VPS bought outside the country** with a domain registered through an **off-shore registrar**. On that same VPS, **enable Warp from inside TzGate** so the server's egress goes through Cloudflare Warp and the real VPS IP is never exposed downstream.
- Share configs **privately** (1-to-1, end-to-end-encrypted chat) — not in groups, not on websites, not pinned in channels.
- Use `vpntz-enc://` lock features (expiration, device-bind, hide-resolvers, no-resharing) to **slow down** abuse, not to make a config publicly shareable.

> ⚠️ A `vpntz://` link contains your credentials. Never post yours in a public group.
> ⚠️ A `vpntz-enc://` link is **not** safe to post publicly either — the VPN-TZ app can decrypt it. The lock controls usage, not secrecy. The encryption protects the *credentials* on the wire; it does **not** protect the *domain identity* once the link is in the open.

---

## 14. Donations

Development is unpaid. If VPN-TZ helps you, donation addresses (BEP-20 / ERC-20 / Arbitrum, Monero, etc.) are kept up to date in the project README on GitHub: <https://github.com/vpntz/vpn-tz#donations>

---

## 15. Stay safe

- Only download VPN-TZ from the **official Telegram channel** or **GitHub**.
- Verify the APK signature if you're paranoid (signing fingerprint is on the GitHub release page).
- Never share your `vpntz://` link with strangers.
- Use **encrypted export** for backups.
- Verify your server is genuine with **Prism** (server-verified scan) — works with TzGate.
- If you run your own server: never hand out `root` or any shell-login account as a VPN credential — always create dedicated TzGate users.

Repository: https://github.com/vpntz/vpn-tz
Source: https://github.com/vpntz/vpn-tz

---
---

# راهنمای کامل VPN-TZ (فارسی)

> مخزن رسمی: https://github.com/vpntz/vpn-tz
> سورس‌کد: https://github.com/vpntz/vpn-tz
> VPN-TZ در گوگل‌پلی، اپ‌استور یا هیچ مارکت دیگری نیست. فقط از کانال رسمی یا گیت‌هاب دانلود کنید.

---

## فهرست

- ۱. VPN-TZ چیست و چرا ساخته شده
- ۲. تونل‌های پشتیبانی‌شده
- ۳. نسخه‌ها: Full و Lite
- ۴. نصب APK
- ۵. سرور لازم است (با TzGate)
- ۶. اولین پروفایل
- ۷. اتصال و قطع
- ۸. اسکنر DNS
...
VPN-TZ یک ابزار رایگان و **source-available** (سورس‌کدش قابل مطالعه است، اما اجازه‌ی توزیع مجدد ندارد) برای دور زدن سانسور و بازرسی شبکه است. نسخه‌ی اصلی برای اندروید است و یک کلاینت CLI هم برای مک، لینوکس و ویندوز ارائه می‌شود. VPN-TZ ترافیک اینترنت شما را از داخل پروتکل‌های متفاوتی مثل DNS، QUIC، SSH، HTTPS، VLESS یا Tor عبور می‌دهد تا حتی در شبکه‌هایی که اینترنت در آن‌ها فیلتر، محدود یا با DPI بازرسی می‌شود، بتوانید آزادانه به وب دسترسی پیدا کنید.

برخلاف یک VPN معمولی که تنها یک پروتکل دارد، VPN-TZ چند **حامل (transport)** مختلف ارائه می‌کند تا متناسب با شرایط شبکه‌ای که در آن گیر افتاده‌اید بتوانید بهترین مسیر را انتخاب کنید:
...
از نظر فلسفه، VPN-TZ در همان خانواده‌ی پروژه‌های ضد سانسور قرار می‌گیرد که [Tor](https://torproject.org)، [Psiphon](https://psiphon.ca) و [Outline VPN](https://getoutline.org) متعلق به آن هستند؛ با این تفاوت مهم که VPN-TZ **فقط کلاینت** است: شبکه یا سرور مرکزی متعلق به یک سازمان خاص وجود ندارد و سرور را یا خودتان راه‌اندازی می‌کنید (با [TzGate](https://github.com/vpntz/vpn-tz/tzgate) فقط چند دقیقه طول می‌کشد) یا از طریق فردی مورد اعتماد دریافت می‌کنید. این مدل سه مزیت اصلی دارد:
...
سورس‌کد VPN-TZ به‌صورت کامل روی گیت‌هاب در دسترس است (تحت لایسنس source-available — قابل مطالعه و مشارکت، اما بدون اجازه‌ی توزیع مجدد یا انتشار در استورها) و در هیچ مارکت اپ (گوگل‌پلی، اپ‌استور و…) منتشر نمی‌شود؛ تنها منبع رسمی، مخزن [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz) و مخزن گیت‌هاب پروژه است.
...
VPN-TZ در دو نسخه عرضه می‌شود — تفاوت در حجم APK و پروتکل‌های موجود است.
...
۱. کانال رسمی [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz) یا صفحه‌ی Releases در گیت‌هاب را باز کنید.
...
۶. تنظیم پیشنهادی یک‌باره: **تنظیمات → بهینه‌سازی باتری → VPN-TZ را بهینه نکن** تا اندروید آن را در پس‌زمینه نکشد.

> ⚠️ اگر VPN-TZ را در گوگل‌پلی، اپ‌استور یا هر مارکت دیگری دیدید، **مال ما نیست**؛ نصب نکنید.

---

## ۵. سرور لازم است — از TzGate استفاده کنید

VPN-TZ *کلاینت* است. برای کار به یک **سرور** نیاز دارد که تونل سازگار را اجرا کند.

روش رسمی و پشتیبانی‌شده برای راه‌اندازی سرور **TzGate** است — یک نصاب لینوکسی تک‌دستوری که سمت سرور تمام پروتکل‌های VPN-TZ را راه می‌اندازد.

> https://github.com/vpntz/vpn-tz/tzgate

راهنمای جداگانه‌ی TzGate در کنار همین سند منتشر شده است. خلاصه: یک VPS ۵ دلاری بگیرید، دامنه‌اش را به آن وصل کنید، یک دستور نصب اجرا کنید، یک کاربر بسازید، و TzGate یک لینک `vpntz://` می‌دهد که مستقیم در اپ پیست می‌شود.

اگر سرور خودتان را راه نمی‌اندازید، می‌توانید:
- از دوستی که TzGate دارد لینک `vpntz://` بگیرید، یا
- از لینک تستی عمومی که گاهی در [vpntz/vpn-tz](https://github.com/vpntz/vpn-tz) منتشر می‌شود استفاده کنید.
...
### الف) چسباندن لینک `vpntz://` (ساده‌ترین)

۱. VPN-TZ را باز کنید → دکمه‌ی **+** پایین سمت راست.
۲. **Import from URI** را انتخاب کنید.
۳. لینک `vpntz://...` را پیست کنید.
...
- نوار اعلان را پایین بکشید → **کاشی Quick Settings** VPN-TZ را بزنید (اولین بار باید از پنل کاشی‌ها اضافه‌اش کنید).
...
- **Transport** — فقط WebSocket (TCP خام در اپ پشتیبانی نمی‌شود — VLESS در VPN-TZ به‌عنوان تونل پشت CDN در نظر گرفته شده)
- **WS Path** — مثلاً `/`، `/vless`
...
حالت SOCKS5 در VPN-TZ یک **passthrough** ساده است — فقط یک listener محلی روی گوشی باز می‌کند و هر درخواست را به پراکسی SOCKS5 ریموت زنجیر می‌کند. **هیچ تست end-to-end اولیه‌ای انجام نمی‌شود.** وضعیت «Connected» فقط دو چیز را تأیید می‌کند:
...
| اندروید VPN-TZ را در پس‌زمینه می‌کشد | تنظیمات → **بهینه‌سازی باتری** → VPN-TZ را بهینه نکن |
...
- **اشتراک یک پروفایل**: روی پروفایل بزنید → منو → **Export** → URI `vpntz://` یا JSON.
...
VPN-TZ سه نوع URI برای خروجی دارد. این‌ها **هم‌ارز نیستند** — مدل امنیتی هرکدام فرق می‌کند.

| فرمت | چیست | کلید رمزنگاری |
|---|---|---|
| `vpntz://` | پروفایل ساده‌ی base64 | ندارد — هرکسی می‌تواند بخواند |
| `vpntz-enc://` | پروفایل تکیِ **قفل‌شده** (مخصوص گیرنده‌ی خاص) | کلید درون‌برنامه‌ای VPN-TZ |
| `vpntz-bundle-enc://` | بسته‌ی چندپروفایلی، رمز شده با رمز عبور | از رمز عبور خود شما مشتق می‌شود |

**`vpntz://`** — هرکسی با base64 می‌تواند بخواند. فقط برای گیرنده‌ی مورد اعتماد.

**`vpntz-enc://`** — وقتی یک پروفایل تکی را با قفل خروجی می‌گیرید (رمز، تاریخ انقضا اختیاری، قید به یک دستگاه، مخفی‌سازی resolverها، عدم اجازه‌ی بازنشر). محتوا با AES-256-GCM رمز می‌شود، **اما کلید AES درون اپ VPN-TZ جاسازی شده** و از رمز شما مشتق نمی‌شود. رمزی که می‌گذارید توسط اپ گیرنده بررسی می‌شود و فقط **نحوه‌ی استفاده** را کنترل می‌کند (فقط استفاده، عدم ویرایش، انقضا، قید به دستگاه، مخفی‌بودن resolverها). محرمانگی کانفیگ در برابر کسی که اپ را نصب دارد تأمین نمی‌شود. این را یک ابزار **کنترل اشتراک‌گذاری** بدانید، نه محرمانگی واقعی.

**`vpntz-bundle-enc://`** — توسط **Export All (Encrypted)** ساخته می‌شود. این یکی واقعاً با رمز عبور خودتان رمزنگاری می‌شود و برای پشتیبان‌گیری مناسب است.

### ⚠️ حتی کانفیگ رمزشده هم نباید در فضای عمومی منتشر شود

رمزنگاریِ بایت‌های کانفیگ، **هویت** سرور، دامنه یا CDN شما را پنهان نمی‌کند. به محض اینکه یک لینک `vpntz-enc://` در کانال عمومی منتشر شود:

- هرکسی که APK VPN-TZ را داشته باشد می‌تواند آن را باز کند و دامنه، IP، SNI، CDN host و credential را ببیند.
...
- برای پخش گسترده‌تر، روی **VPS خریداری‌شده از خارج کشور** و دامنه‌ی ثبت‌شده در **ریجیسترار خارج از کشور** میزبانی کنید. روی همان VPS، **Warp را از داخل TzGate فعال کنید** تا ترافیک خروجی سرور از پشت Cloudflare Warp عبور کند و IP واقعی VPS لو نرود.
- کانفیگ را **خصوصی** بفرستید (تک به تک، در چت رمزنگاری‌شده‌ی end-to-end) — نه در گروه، نه در سایت، نه پین‌شده در کانال.
- قابلیت‌های قفل `vpntz-enc://` (انقضا، قید به دستگاه، مخفی‌بودن resolverها، عدم بازنشر) را برای **کند کردن سوءاستفاده** به کار ببرید، نه برای اینکه کانفیگ را عمومی کنید.

> ⚠️ لینک `vpntz://` حاوی اطلاعات حساس شماست؛ هرگز آن را در گروه عمومی نگذارید.
>
> ⚠️ لینک `vpntz-enc://` هم **برای انتشار عمومی امن نیست**: هر کسی که اپ VPN-TZ را داشته باشد می‌تواند آن را رمزگشایی کند. قفل، فقط *نحوه‌ی استفاده* را محدود می‌کند و **محرمانگی محتوا را تضمین نمی‌کند**. رمزنگاری از credentialها در حین انتقال محافظت می‌کند، اما به محض اینکه لینک علنی شد، **هویت دامنه و سرور شما لو می‌رود** و رمزنگاری نمی‌تواند جلویش را بگیرد.

---

## ۱۴. حمایت مالی

توسعه‌ی VPN-TZ داوطلبانه و بدون درآمد است. اگر این اپ به شما کمک کرده، آدرس‌های دونیت (BEP-20 / ERC-20 / Arbitrum، Monero و…) همیشه به‌روز در README پروژه روی گیت‌هاب نگه‌داری می‌شوند: <https://github.com/vpntz/vpn-tz#donations>

---

## ۱۵. نکات امنیتی

- VPN-TZ را فقط از **کانال رسمی تلگرام** یا **گیت‌هاب** دانلود کنید.
- لینک `vpntz://` خود را با غریبه‌ها به اشتراک نگذارید.
- موقع پشتیبان‌گیری از **Export رمزشده** استفاده کنید.
- اصالت سرور را با **Prism** (اسکن سرور-تأییدشده) اعتبارسنجی کنید — با TzGate کار می‌کند.
- اگر سرور خودتان را اجرا می‌کنید: هیچ‌وقت `root` یا هر اکانت shell را به‌عنوان credential VPN ندهید — همیشه با TzGate کاربر مخصوص بسازید.

مخزن: https://github.com/vpntz/vpn-tz
سورس: https://github.com/vpntz/vpn-tz
