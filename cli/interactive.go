...
func runInteractive() {
	...
	for {
		...
		case "0", "q", "exit":
			...
			return
		default:
			// If they pasted a vpntz:// or vpntz-enc:// URI directly, treat as connect
			if isVpnTzURI(choice) {
...
func interactiveConnect() {
	...
	fmt.Println("  ── Connect ──────────────────────────────────────")
	fmt.Println()
	uri := prompt("  Paste vpntz:// or vpntz-enc:// config: ")
	if uri == "" {
		return
	}
	if !isVpnTzURI(uri) {
		fmt.Println("  Invalid config. Must start with vpntz:// or vpntz-enc://")
		waitExit()
		return
	}
...
func interactiveConnectWithURI(uri string) {
	...
	case "ss", "slipstream_ssh", "snowflake", "naive":
		...
		fmt.Println("  VPN-TZ CLI supports DNSTT, NoizDNS, VayDNS, SSH, SOCKS5, and DoH tunnel types.")
		fmt.Println("  Use the VPN-TZ app for other tunnel types.")
		waitExit()
		return
	case "socks5", "direct_socks":
		if profile.SOCKSUser == "" && profile.SSHUser == "" {
			...
			fmt.Println("  Add credentials to your config and re-export, or use the VPN-TZ app.")
			waitExit()
			return
		}
	}
...
func interactiveScan(withE2E bool) {
	...
	if withE2E {
		configURI := promptDefault("  vpntz:// config (for domain+key, or blank to enter manually)", "")
		if configURI != "" && isVpnTzURI(configURI) {
	...
	case "2":
		...
		// Write to temp file
		tmpFile, err := os.CreateTemp("", "vpntz-ips-*.txt")
		if err != nil {
...
func interactiveVerifyScan() {
	...
	configURI := promptDefault("  vpntz:// config (or blank to enter manually)", "")
	if configURI != "" && isVpnTzURI(configURI) {
...
func isVpnTzURI(s string) bool {
	ls := strings.ToLower(s)
	return strings.HasPrefix(s, "vpntz://") || strings.HasPrefix(s, "vpntz-enc://") ||
