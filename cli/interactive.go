...
func runInteractive() {
	...
	for {
		...
		case "0", "q", "exit":
			...
			return
		default:
			// If they pasted a tzvpn:// or tzvpn-enc:// URI directly, treat as connect
			if isTZVPNURI(choice) {
...
func interactiveConnect() {
	...
	fmt.Println("  ── Connect ──────────────────────────────────────")
	fmt.Println()
	uri := prompt("  Paste tzvpn:// or tzvpn-enc:// config: ")
	if uri == "" {
		return
	}
	if !isTZVPNURI(uri) {
		fmt.Println("  Invalid config. Must start with tzvpn:// or tzvpn-enc://")
		waitExit()
		return
	}
...
func interactiveConnectWithURI(uri string) {
	...
	case "ss", "slipstream_ssh", "snowflake", "naive":
		...
		fmt.Println("  TZVPN CLI supports DNSTT, NoizDNS, VayDNS, SSH, SOCKS5, and DoH tunnel types.")
		fmt.Println("  Use the TZVPN app for other tunnel types.")
		waitExit()
		return
	case "socks5", "direct_socks":
		if profile.SOCKSUser == "" && profile.SSHUser == "" {
			...
			fmt.Println("  Add credentials to your config and re-export, or use the TZVPN app.")
			waitExit()
			return
		}
	}
...
func interactiveScan(withE2E bool) {
	...
	if withE2E {
		configURI := promptDefault("  tzvpn:// config (for domain+key, or blank to enter manually)", "")
		if configURI != "" && isTZVPNURI(configURI) {
	...
	case "2":
		...
		// Write to temp file
		tmpFile, err := os.CreateTemp("", "tzvpn-ips-*.txt")
		if err != nil {
...
func interactiveVerifyScan() {
	...
	fmt.Println()
	fmt.Println("  Requires the server public key (from tzvpn:// config")
	...
	configURI := promptDefault("  tzvpn:// config (or blank to enter manually)", "")
	if configURI != "" && isTZVPNURI(configURI) {
