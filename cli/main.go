// vpntz is a cross-platform CLI client for VPN-TZ/NoizDNS.
//
// Usage:
//
//	vpntz [--dns RESOLVER] [--port PORT] [--host HOST] [--max-query-size BYTES] vpntz://BASE64ENCODED...
//
// It decodes the vpntz:// URI, extracts connection parameters,
...
func parseURI(uri string) (*Profile, error) {
	...
	default:
		return nil, fmt.Errorf("invalid URI scheme, expected vpntz://, vpntz-enc://, or vless://")
	}
...
func connectWithParams(uri string, portOverride int, hostOverride string, dnsOverride string, utlsOverride string, forceDirectMode bool, querySize int, maxQnameLen int, resolverMode string, spreadCount int, vOpts vaydnsCLIOverrides) {
	...
	case "ss", "slipstream_ssh", "snowflake", "naive":
		fmt.Fprintf(os.Stderr, "  Error: This config uses tunnel type %q which is not supported by the CLI.\n"+
			"  VPN-TZ CLI supports DNSTT, NoizDNS, VayDNS, SSH, SOCKS5, VLESS, Hysteria2, and DoH tunnel types.\n"+
			"  Use the VPN-TZ app for other tunnel types.\n", profile.TunnelType)
		return
	default:
	...
	if profile.PublicKey == "" {
		fmt.Fprintln(os.Stderr, "  Error: Config is missing the server's public key.\n"+
			"  The vpntz:// URI must include the server's Noise public key.\n"+
			"  Ask your server admin for a config that includes the public key,\n"+
			"  or re-export the config from the VPN-TZ app.")
		return
	}
	if profile.Domain == "" {
...
func runScanCommand(args []string) {
	...
	// If --config is provided, extract domain, pubkey, tunnel mode, and ssh mode from vpntz:// URI
	var sshMode bool
	...
	if verifyMode {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: verify mode requires --pubkey or --config with a vpntz:// URI")
			return
		}
	...
	if e2eOnly {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: E2E-only mode requires --pubkey or --config with a vpntz:// URI")
			return
		}
	...
	if e2eEnabled {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: E2E testing requires --pubkey or --config with a vpntz:// URI")
			return
		}
		e2eConfig = &E2EConfig{
			TunnelDomain:   domain,
...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, `VPN-TZ CLI %s - Tunnel proxy (DNSTT, NoizDNS, VayDNS, SSH, SOCKS5, DoH)

Usage:
  %s [options] vpntz://BASE64...
  %[2]s [options] vpntz-enc://BASE64...
...
Options (scan):
  ...
  --config URI        Extract domain/pubkey/mode from vpntz:// URI (auto-enables E2E)
...
Examples:
  %[2]s vpntz://BASE64...
  %[2]s --utls Chrome_120 vpntz://BASE64...
  %[2]s --dns 1.1.1.1 vpntz://BASE64...
  %[2]s --dns <server-ip> --direct --port 9050 vpntz://BASE64...
  %[2]s --max-query-size 80 vpntz://BASE64...
  %[2]s --host 0.0.0.0 vpntz://BASE64...
  %[2]s scan --domain t.example.com --ips resolvers.txt
  %[2]s scan --domain t.example.com --ip 8.8.8.8
  %[2]s scan --config vpntz://BASE64... --ips resolvers.txt
  %[2]s scan --domain t.example.com --ips ips.txt --e2e --pubkey HEXKEY
  %[2]s scan --config vpntz://BASE64... --ips resolvers.txt --verify
  %[2]s scan --config vpntz://BASE64... --ips resolvers.txt --e2e-only
