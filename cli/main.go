// tzvpn is a cross-platform CLI client for TZVPN/NoizDNS.
//
// Usage:
//
//	tzvpn [--dns RESOLVER] [--port PORT] [--host HOST] [--max-query-size BYTES] tzvpn://BASE64ENCODED...
//
// It decodes the tzvpn:// URI, extracts connection parameters,
...
func parseURI(uri string) (*Profile, error) {
	...
	default:
		return nil, fmt.Errorf("invalid URI scheme, expected tzvpn://, tzvpn-enc://, or vless://")
	}
...
func connectWithParams(uri string, portOverride int, hostOverride string, dnsOverride string, utlsOverride string, forceDirectMode bool, querySize int, maxQnameLen int, resolverMode string, spreadCount int, vOpts vaydnsCLIOverrides) {
	...
	if profile.PublicKey == "" {
		fmt.Fprintln(os.Stderr, "  Error: Config is missing the server's public key.\n"+
			"  The tzvpn:// URI must include the server's Noise public key.\n"+
			"  Ask your server admin for a config that includes the public key,\n"+
			"  or re-export the config from the TZVPN app.")
		return
	}
	if profile.Domain == "" {
...
func runScanCommand(args []string) {
	...
	// If --config is provided, extract domain, pubkey, tunnel mode, and ssh mode from tzvpn:// URI
	var sshMode bool
	...
	if verifyMode {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: verify mode requires --pubkey or --config with a tzvpn:// URI")
			return
		}
	...
	if e2eOnly {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: E2E-only mode requires --pubkey or --config with a tzvpn:// URI")
			return
		}
	...
	if e2eEnabled {
		if pubkey == "" {
			fmt.Fprintln(os.Stderr, "  Error: E2E testing requires --pubkey or --config with a tzvpn:// URI")
			return
		}
		e2eConfig = &E2EConfig{
			TunnelDomain:   domain,
...
func printUsage() {
	...
	fmt.Fprintf(os.Stderr, `TZVPN CLI %s - Tunnel proxy (DNSTT, NoizDNS, VayDNS, SSH, SOCKS5, DoH)

Usage:
  %s [options] tzvpn://BASE64...
  %[2]s [options] tzvpn-enc://BASE64...
...
Options (scan):
  ...
  --config URI        Extract domain/pubkey/mode from tzvpn:// URI (auto-enables E2E)
...
Examples:
  %[2]s tzvpn://BASE64...
  %[2]s --utls Chrome_120 tzvpn://BASE64...
  %[2]s --dns 1.1.1.1 tzvpn://BASE64...
  %[2]s --dns <server-ip> --direct --port 9050 tzvpn://BASE64...
  %[2]s --max-query-size 80 tzvpn://BASE64...
  %[2]s --host 0.0.0.0 tzvpn://BASE64...
  %[2]s scan --domain t.example.com --ips resolvers.txt
  %[2]s scan --domain t.example.com --ip 8.8.8.8
  %[2]s scan --config tzvpn://BASE64... --ips resolvers.txt
  %[2]s scan --domain t.example.com --ips ips.txt --e2e --pubkey HEXKEY
  %[2]s scan --config tzvpn://BASE64... --ips resolvers.txt --verify
  %[2]s scan --config tzvpn://BASE64... --ips resolvers.txt --e2e-only
