...
const (
	WarpDir     = "/etc/cybergate/warp"
	WarpConf    = "/etc/cybergate/warp/wg0.conf"
	ProfileFile = "/etc/cybergate/warp/wgcf-profile.conf" // legacy wgcf profile
	ServiceName = "cybergate-warp"
	RouteTable  = 200
...
func Setup(cfg *config.Config, log func(string)) error {
	...
	// upgrading via `cybergate install` get the asymmetric-routing fix
	// without a restart.
	if IsRunning() {
...
func collectUserUIDs(cfg *config.Config) []int {
	...
	// NaiveProxy's user (cybergate-naive) is intentionally NOT routed
	// through WARP. Caddy is just an HTTPS-wrapped TCP forwarder — actual
	// proxied traffic terminates on 127.0.0.1:1080 (cybergate-socks) or
...
func ensureSocksUser() error {
	...
	// Ensure the cybergate group exists
	_ = tryRun("groupadd", "--system", config.SystemGroup)
...
Description=CyberGate WARP (Cloudflare WireGuard)
...
// resolvedDropIn is the systemd-resolved drop-in cybergate installs so that
// WARP-routed services can resolve DNS. Removed on Uninstall.
const resolvedDropIn = "/etc/systemd/resolved.conf.d/cybergate-warp-dns.conf"
...
// any WARP-routed cybergate service (NaiveProxy, per-user SOCKS egress, etc.)
