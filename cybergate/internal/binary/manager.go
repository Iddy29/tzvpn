...
const releaseBaseURL = "https://github.com/Iddy29/tzvpn/tree/main/cybergate/releases"
...
const (
	...
	repoAPI            = "https://api.github.com/repos/anonvector/cybergate/releases"
)

// DownloadBase returns the base URL for cybergate binary downloads.
...
func CheckUpdate() (newVersion string, downloadURL string, err error) {
	...
	// Find matching asset
	target := fmt.Sprintf("cybergate-%s-%s", runtime.GOOS, runtime.GOARCH)
...
func Download(url string) (string, error) {
	tmp, err := os.CreateTemp("", "cybergate-update-*")
	if err != nil {
		return "", err
	}
	defer tmp.Close()
