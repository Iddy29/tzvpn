...
const (
	iptablesRulesPath = "/etc/iptables/cybergate-rules.v4"
	iptablesUnitPath  = "/etc/systemd/system/cybergate-iptables.service"
)
...
Description=Restore cybergate iptables rules
...
func iptablesPersist() error {
	...
	if string(existing) != unit {
		...
		if err := run("systemctl", "enable", "cybergate-iptables.service"); err != nil {
			return err
		}
	}
	return nil
...
// so that cybergate's DNS router can bind to it.
func DisableResolvedStub() error {
	...
	conf := "[Resolve]\nDNSStubListener=no\n"
	if err := os.WriteFile("/etc/systemd/resolved.conf.d/cybergate-no-stub.conf", []byte(conf), 0644); err != nil {
