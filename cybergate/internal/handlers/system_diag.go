...
import (
	...
	"github.com/Iddy29/tzvpn/tree/main/cybergate/internal/version"
...
func handleSystemDiag(ctx *actions.Context) error {
	...
	out.Print("")
	out.Print("  CyberGate Diagnostics")
	...
	check("System user (cybergate)", err == nil, boolStr(err == nil, "exists", "missing — run install"))
	...
	for _, t := range cfg.Tunnels {
		// Direct transports use system services (sshd, microsocks), not cybergate-managed ones
	...
	// DNS router
	if hasDNSTunnels {
		routerExists := service.Exists("cybergate-dnsrouter")
		if routerExists {
			...
			if !routerOK {
				showRecentLogs(out, "cybergate-dnsrouter")
			}

			enabled := isServiceEnabled("cybergate-dnsrouter")
	...
	// SOCKS5 proxy
	socksExists := service.Exists("cybergate-socks5")
	if socksExists {
		status, _ := service.Status("cybergate-socks5")
		socksOK := status == "active"
		check("SOCKS5 proxy service", socksOK,
			boolStr(socksOK, "active", status))
		if !socksOK {
			showRecentLogs(out, "cybergate-socks5")
		}
	...
	if cfg.Warp.Enabled {
		...
		warpExists := service.Exists("cybergate-warp")
		if warpExists {
			status, _ := service.Status("cybergate-warp")
			warpOK := status == "active"
			check("WARP service", warpOK,
				boolStr(warpOK, "active", status))
			if !warpOK {
				showRecentLogs(out, "cybergate-warp")
			}
		} else {
		...
		if len(expectedUIDs) == 0 {
			...
			out.Info("  Hint: run 'cybergate restart' or re-enable WARP to fix missing rules")
		}

		// SOCKS5 service user
		if service.Exists("cybergate-socks5") {
			actualUser := service.GetUser("cybergate-socks5")
			userOK := actualUser == warp.SocksUser
			check("SOCKS5 service user", userOK,
				boolStr(userOK,
					fmt.Sprintf("%s (correct)", actualUser),
					fmt.Sprintf("%s — should be %s; run 'cybergate restart' to fix", actualUser, warp.SocksUser)))
		}
	...
	// ── Orphaned Services ───────────────────────────────────
	allSvc := service.ListCybergateServices()
	knownSvc := map[string]bool{
		"cybergate-dnsrouter": true,
		"cybergate-socks5":    true,
		"cybergate-warp":      true,
		"cybergate-iptables":  true,
	}
	for _, t := range cfg.Tunnels {
...
func isResolvedStubDisabled() bool {
	data, err := os.ReadFile("/etc/systemd/resolved.conf.d/cybergate-no-stub.conf")
	if err != nil {
		return false
	}
...
func collectExpectedWarpUIDs(cfg *config.Config) map[string]int {
	...
	// cybergate-naive (Caddy) is intentionally NOT WARP-routed — see
