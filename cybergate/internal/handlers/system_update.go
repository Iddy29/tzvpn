...
import (
	...
	"github.com/Iddy29/tzvpn/tree/main/cybergate/internal/transport"
...
func handleSystemUpdate(ctx *actions.Context) error {
	...
	downloadURL := fmt.Sprintf("%s/cybergate-%s-%s", base, runtime.GOOS, runtime.GOARCH)
...
func runPostUpdate(ctx *actions.Context) error {
	...
	if service.Exists("cybergate-socks5") {
		...
		if setupErr != nil {
			...
			out.Success("  cybergate-socks5 regenerated and restarted")
		}
	}
