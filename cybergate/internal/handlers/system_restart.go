...
import (
	...
	"github.com/Iddy29/tzvpn/tree/main/cybergate/internal/network"
	"github.com/Iddy29/tzvpn/tree/main/cybergate/internal/proxy"
...
func handleSystemRestart(ctx *actions.Context) error {
	...
	if service.Exists("cybergate-socks5") {
		...
		if service.GetUser("cybergate-socks5") != expectedUser {
			if cfg.Warp.Enabled {
			...
			if socksErr != nil {
				out.Warning(fmt.Sprintf("Failed to restart cybergate-socks5: %v", socksErr))
			} else {
				out.Success("  cybergate-socks5 restarted (fixed service user)")
			}
		} else {
			if err := service.Restart("cybergate-socks5"); err != nil {
				out.Warning(fmt.Sprintf("Failed to restart cybergate-socks5: %v", err))
			} else {
				out.Success("  cybergate-socks5 restarted")
			}
		}
	}
	...
	if service.Exists("cybergate-dnsrouter") {
		if err := service.Restart("cybergate-dnsrouter"); err != nil {
			out.Warning(fmt.Sprintf("Failed to restart cybergate-dnsrouter: %v", err))
		} else {
			out.Success("  cybergate-dnsrouter restarted")
		}
	}
