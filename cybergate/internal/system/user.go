...
// EnsureUser creates the cybergate system user and groups if they don't exist.
...
// RemoveUser removes the cybergate system user and groups.
...
// EnsureDir creates a directory owned by the cybergate user.
...
// AddSSHUser creates a system user in the cybergate-ssh group.
...
// ListSSHUsers returns all users in the cybergate-ssh group.
...
# CyberGate global SSH settings
MaxStartups 100:30:300
MaxSessions 100
TCPKeepAlive no
`
	const matchBlock = `
# CyberGate SSH tunnel users
Match Group cybergate-ssh
...
`
	...
	if !strings.Contains(content, "# CyberGate global SSH settings") {
		// Insert before the first Match block or CyberGate block, or append
		if idx := strings.Index(content, "# CyberGate SSH tunnel users"); idx >= 0 {
	...
	if strings.Contains(content, "Match Group cybergate-ssh") {
