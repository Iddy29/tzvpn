# CyberGate — Complete Operator Guide

> The official server for the [SlipNet](https://github.com/anonvector/SlipNet) Android app.
> Source: https://github.com/Iddy29/tzvpn/tree/main/cybergate
...
You need one domain (e.g., `example.com`) that you control. Add these records before installing CyberGate:
...
- Downloads the CyberGate binary plus `dnstt-server`, `slipstream-server`, `vaydns-server`, and `caddy-naive`.
- Installs systemd services (`cybergate-dnsrouter`, `cybergate-socks5`).
- Sets up the firewall rules where applicable.

**Offline install** (when your VPS can't reach GitHub directly):

1. On your laptop, download from https://github.com/Iddy29/tzvpn/tree/main/cybergate/releases.
2. SCP the binaries to the server: `scp cybergate-* user@server:/tmp/cybergate/`
3. On the server: `sudo cybergate install --bin-dir /tmp/cybergate`
...
CyberGate has two interfaces:

**Interactive TUI** — `sudo cybergate` with no arguments. A friendly menu that walks you through tunnels, users, stats, diagnostics, etc. Best for first-time setup and exploration.

**CLI subcommands** — for scripting and automation. Every interactive option has a CLI equivalent:

```
cybergate install                Install dependencies, set up server
cybergate uninstall              Remove everything
cybergate update                 Self-update + restart services
cybergate restart                Restart all services
cybergate users                  Add / list / remove users (interactive)
cybergate users add              Add one user
cybergate users bulk_add         Add many users with random passwords
cybergate users list             List users + per-tunnel configs
cybergate stats                  Live dashboard
cybergate diag                   Run diagnostic checks
cybergate mtu [value]            Set MTU on all DNS tunnels at once

cybergate tunnel add             Add a tunnel
cybergate tunnel edit [tag]      Edit settings (rename, MTU, keys)
cybergate tunnel remove [tag]    Remove one tunnel
cybergate tunnel remove --all    Remove all tunnels
cybergate tunnel start [tag]     Start a tunnel
cybergate tunnel stop [tag]      Stop a tunnel
cybergate tunnel status [tag]    Show status / details
cybergate tunnel share [tag]     Print slipnet:// URI + QR
cybergate tunnel logs [tag]      Show systemd logs

cybergate router status          DNS routing config
cybergate router mode            Switch single ↔ multi mode
cybergate router switch          Change active tunnel (single mode)

cybergate config export          Export full config (JSON)
cybergate config import          Import config back in
```
...
4. Creates a systemd service `cybergate-mytunnel` that starts on boot.

**Confirm it's running:**

```bash
sudo cybergate tunnel status
```
...
CyberGate manages six transports. Pick based on the network your users have to escape from.
...
sudo cybergate tunnel add --transport vaydns --backend socks \
  --tag myvaydns --domain v.example.com

# VayDNS with all tuning knobs
sudo cybergate tunnel add --transport vaydns --backend both \
  --tag myvaydns --domain v.example.com \
...
sudo cybergate tunnel add --transport naive --backend socks \
...
sudo cybergate tunnel add --transport stuntls --tag mytls
...
sudo cybergate tunnel add --transport external --tag my-proto \
...
Users in CyberGate are **global**. The same `username:password` authenticates against every tunnel, regardless of transport. The protocol is a property of the **tunnel**, not the user.

> ⚠️ **Never hand out the server's `root` (or any sudoer / shell login) account as a VPN credential.** CyberGate users are isolated SOCKS/SSH-forwarding accounts with no shell access — losing one only loses tunnel access. A leaked `root` account loses the whole server: your keys, your other users' credentials, and any other service running on the box. Always create dedicated CyberGate users with `cybergate users add` (or `bulk_add`) for everyone you give a config to, including yourself.

**Add one user (interactive):**

```bash
sudo cybergate users add
```
...
You can run as many tunnels as you want. CyberGate auto-switches into **multi-mode** when you create a second DNS tunnel — the DNS router on port 53 dispatches queries by domain to the correct tunnel.

Example: DNSTT + VayDNS + NaiveProxy at the same time:

```bash
sudo cybergate tunnel add --transport dnstt   --backend socks --tag dnstt1   --domain t.example.com
sudo cybergate tunnel add --transport vaydns  --backend socks --tag vaydns1  --domain v.example.com
sudo cybergate tunnel add --transport naive   --backend socks --tag naive1   --domain example.com --email you@example.com
```

Each tunnel becomes its own systemd service (`cybergate-dnstt1`, etc.). Manage them like any service:

```bash
sudo cybergate tunnel status
sudo cybergate tunnel start vaydns1
sudo cybergate tunnel stop vaydns1
sudo cybergate tunnel logs naive1
```
...
```bash
sudo cybergate router status      # see current mode
sudo cybergate router mode        # switch between single and multi
sudo cybergate router switch      # change active tunnel (single mode only)
```
...
Enable it from the interactive menu (**WARP** option). CyberGate sets up a `wireguard-go` link to WARP and routes user traffic through it.
...
```bash
sudo cybergate tunnel logs mytunnel
```

**Restart everything** (DNS router, SOCKS, all tunnels):

```bash
sudo cybergate restart
```
...
```bash
sudo cybergate tunnel edit --tag mydnstt --new-tag my-tunnel  # rename
sudo cybergate tunnel edit --tag mydnstt --mtu 1232           # change MTU
sudo cybergate tunnel status --tag mydnstt                    # keys, port, MTU
```
...
```bash
sudo cybergate config export > cybergate-backup.json
```
...
The SlipNet client supports **VLESS** (over WebSocket / TLS, typically fronted through Cloudflare). **CyberGate does not run a VLESS server itself** — VLESS is part of the [Xray / V2Ray](https://github.com/XTLS/Xray-core) ecosystem.

If you want to give your users a VLESS option, you have two paths:

1. **Run Xray separately on the same VPS.** Standard Xray installers (e.g., `Xray-install`, `x-ui`, `3x-ui`) work fine alongside CyberGate, since they typically listen on a different port (or on 443 if CyberGate isn't using NaiveProxy/StunTLS). Your users build a manual VLESS profile in SlipNet using the UUID, domain, and CDN settings from your Xray panel.
2. **Skip VLESS** — CyberGate's six transports already cover most censorship environments. NaiveProxy and StunTLS solve the "443 is the only port left" problem.

VLESS support may land in CyberGate in the future. Track the channel for updates.
...
| Symptom | Try this |
|---------|----------|
| Tunnel won't start | `sudo cybergate diag` — checks ports, keys, DNS records |
| Client connects but no traffic | `sudo cybergate tunnel logs <tag>` for handshake / auth errors |
| Port 53 already in use | `sudo systemctl disable --now systemd-resolved` (then add `nameserver 1.1.1.1` to `/etc/resolv.conf`) |
| Let's Encrypt fails (NaiveProxy) | A record may not have propagated, or port 80 is blocked. Wait, then `sudo cybergate restart` |
...
| User can't authenticate | `sudo cybergate users list` — confirm credentials present; check tunnel has matching backend |
| Slow DNS tunnel | Lower MTU: `sudo cybergate mtu 1200` |
| Multiple tunnels but only one works | `sudo cybergate router status` — confirm multi-mode is active |
| Need to reset a key | `sudo cybergate tunnel edit --tag <tag>` and regenerate; clients will need a fresh URI |

For deeper inspection, every tunnel is a regular systemd service:

```bash
journalctl -u cybergate-mytunnel -n 200 --no-pager
journalctl -u cybergate-dnsrouter -n 200 --no-pager
journalctl -u cybergate-socks5 -n 200 --no-pager
```
...
| Path | Description |
|------|-------------|
| `/etc/cybergate/config.json` | Main configuration (tunnels, users, routing mode) |
...
**systemd units:**
- `cybergate-dnsrouter` — port 53 DNS dispatcher
- `cybergate-socks5` — built-in SOCKS5 proxy
- `cybergate-<tag>` — one per tunnel you create
...
CyberGate source: https://github.com/Iddy29/tzvpn/tree/main/cybergate
SlipNet source: https://github.com/anonvector/SlipNet

---
---

# راهنمای کامل CyberGate برای ادمین (فارسی)

> سرور رسمی اپ اندرویدی [SlipNet](https://github.com/anonvector/SlipNet)
> سورس: https://github.com/Iddy29/tzvpn/tree/main/cybergate
...
CyberGate برای شما این کارها را انجام می‌دهد:
...
به یک دامنه نیاز دارید (مثلاً `example.com`). قبل از نصب CyberGate این رکوردها را اضافه کنید:
...
- باینری CyberGate و `dnstt-server`، `slipstream-server`، `vaydns-server` و `caddy-naive` را دانلود می‌کند.
- سرویس‌های systemd (`cybergate-dnsrouter`، `cybergate-socks5`) را نصب می‌کند.
- در صورت نیاز قواعد فایروال را تنظیم می‌کند.

**نصب آفلاین** (اگر VPS به گیت‌هاب دسترسی ندارد):

۱. روی لپ‌تاپ از https://github.com/Iddy29/tzvpn/tree/main/cybergate/releases دانلود کنید.
۲. باینری‌ها را با SCP به سرور بفرستید: `scp cybergate-* user@server:/tmp/cybergate/`
۳. روی سرور: `sudo cybergate install --bin-dir /tmp/cybergate`
...
CyberGate دو رابط دارد:

**TUI تعاملی** — `sudo cybergate` بدون پارامتر. منوی دوستانه‌ای که شما را در ساخت تونل، کاربر، آمار و عیب‌یابی راه می‌برد. برای نصب اولیه و کاوش بهترین انتخاب است.

**زیردستورهای CLI** — برای اسکریپت و اتوماسیون. هر گزینه‌ی منو یک معادل CLI دارد:

```
cybergate install                نصب وابستگی‌ها و راه‌اندازی سرور
cybergate uninstall              حذف همه‌چیز
cybergate update                 خود-آپدیت + ری‌استارت سرویس‌ها
cybergate restart                ری‌استارت همه سرویس‌ها
cybergate users                  افزودن / لیست / حذف کاربر (تعاملی)
cybergate users add              افزودن یک کاربر
cybergate users bulk_add         افزودن گروهی با پسورد تصادفی
cybergate users list             لیست کاربران + کانفیگ تونل‌ها
cybergate stats                  داشبورد زنده
cybergate diag                   اجرای بررسی‌های تشخیصی
cybergate mtu [value]            تنظیم MTU روی همه‌ی تونل‌های DNS

cybergate tunnel add             افزودن یک تونل
cybergate tunnel edit [tag]      ویرایش (نام، MTU، کلید)
cybergate tunnel remove [tag]    حذف یک تونل
cybergate tunnel remove --all    حذف همه‌ی تونل‌ها
cybergate tunnel start [tag]     استارت تونل
cybergate tunnel stop [tag]      استاپ تونل
cybergate tunnel status [tag]    وضعیت / جزئیات
cybergate tunnel share [tag]     چاپ slipnet:// URI + QR
cybergate tunnel logs [tag]      دیدن لاگ‌های systemd

cybergate router status          وضعیت روتر DNS
cybergate router mode            تعویض single ↔ multi
cybergate router switch          تغییر تونل فعال (single mode)

cybergate config export          خروجی کامل کانفیگ (JSON)
cybergate config import          ایمپورت دوباره
```
...
۴. سرویس systemd `cybergate-mytunnel` می‌سازد که در بوت اجرا می‌شود.
...
CyberGate شش transport را مدیریت می‌کند. بر اساس شبکه‌ای که کاربران از آن می‌خواهند فرار کنند انتخاب کنید.
...
کاربر در CyberGate **سراسری** است. همان `username:password` روی همه‌ی تونل‌ها معتبر است، فارغ از transport. پروتکل ویژگی **تونل** است نه کاربر.

> ⚠️ **هیچ‌وقت اکانت `root` سرور (یا هر اکانت sudoer / لاگین shell) را به‌عنوان credential VPN در اختیار کاربران نگذارید.** کاربر CyberGate یک اکانت ایزوله‌ی SOCKS/SSH-forwarding است که دسترسی shell ندارد — اگر یکی لو برود، فقط دسترسی به تونل از دست می‌رود. اما لو رفتن `root` یعنی از دست رفتن کل سرور: کلیدها، credential سایر کاربران و هر سرویس دیگری که روی سرور است. همیشه برای هر کسی که کانفیگ می‌دهید (حتی خودتان) با `cybergate users add` (یا `bulk_add`) یک کاربر مخصوص بسازید.
...
می‌توانید هر تعداد تونل می‌خواهید اجرا کنید. CyberGate وقتی تونل DNS دوم را می‌سازید خودکار به **multi-mode** سوییچ می‌کند — روتر DNS روی پورت ۵۳ کوئری را بر اساس دامنه به تونل درست می‌فرستد.
...
هر تونل سرویس systemd خودش را دارد (`cybergate-dnstt1` و…). مثل هر سرویس مدیریتش کنید:
...
```bash
sudo cybergate router status      # حالت فعلی
sudo cybergate router mode        # تغییر بین single و multi
sudo cybergate router switch      # تغییر تونل فعال (فقط در single mode)
```
...
از منوی تعاملی گزینه‌ی **WARP** را روشن کنید. CyberGate یک لینک `wireguard-go` به WARP می‌سازد و ترافیک کاربر را از آن عبور می‌دهد.
...
```bash
sudo cybergate tunnel edit --tag mydnstt --new-tag my-tunnel  # تغییر نام
sudo cybergate tunnel edit --tag mydnstt --mtu 1232           # تغییر MTU
sudo cybergate tunnel status --tag mydnstt                    # کلید، پورت، MTU
```
...
۳. CyberGate را روی VPS جدید نصب کنید و بکاپ را ایمپورت کنید.
...
اپ SlipNet از **VLESS** پشتیبانی می‌کند (روی WebSocket / TLS، معمولاً پشت Cloudflare). **اما CyberGate خودش سرور VLESS اجرا نمی‌کند** — VLESS بخشی از اکوسیستم [Xray / V2Ray](https://github.com/XTLS/Xray-core) است.

اگر می‌خواهید به کاربران گزینه‌ی VLESS بدهید، دو راه دارید:

۱. **Xray را جدا روی همان VPS اجرا کنید.** نصاب‌های استاندارد Xray (مثل `Xray-install`، `x-ui`، `3x-ui`) در کنار CyberGate خوب کار می‌کنند، چون معمولاً روی پورت متفاوتی گوش می‌دهند (یا روی ۴۴۳ اگر CyberGate از NaiveProxy/StunTLS استفاده نمی‌کند). کاربر شما در SlipNet به‌صورت دستی پروفایل VLESS می‌سازد با UUID، دامنه، و تنظیمات CDN از پنل Xray شما.
۲. **VLESS را رها کنید** — شش transport CyberGate برای اکثر محیط‌های سانسوری کافی است. NaiveProxy و StunTLS مشکل «فقط ۴۴۳ باز است» را حل می‌کنند.

ممکن است در آینده پشتیبانی VLESS به CyberGate اضافه شود. کانال را برای اپدیت دنبال کنید.
...
| نشانه | راه‌حل |
|-------|------|
| تونل بالا نمی‌آید | `sudo cybergate diag` — پورت‌ها، کلیدها و رکوردهای DNS را بررسی می‌کند |
| کلاینت وصل می‌شود ولی ترافیک نمی‌رود | `sudo cybergate tunnel logs <tag>` برای دیدن خطای handshake / احراز هویت |
...
| Let's Encrypt در NaiveProxy fail می‌شود | احتمالاً رکورد A پراپاگیت نشده یا پورت ۸۰ بسته است. صبر کنید و `sudo cybergate restart` |
...
| کاربر احراز نمی‌شود | `sudo cybergate users list` — وجود credential را بررسی کنید؛ مطمئن شوید تونل backend متناظر دارد |
| تونل DNS کند است | MTU را پایین بیاورید: `sudo cybergate mtu 1200` |
| چند تونل ساخته‌ام ولی فقط یکی کار می‌کند | `sudo cybergate router status` — تأیید کنید multi-mode فعال است |
| نیاز به ریست کلید دارم | `sudo cybergate tunnel edit --tag <tag>` و کلید جدید بسازید؛ کلاینت‌ها به URI جدید نیاز دارند |
...
| مسیر | توضیح |
|------|------|
| `/etc/cybergate/config.json` | کانفیگ اصلی (تونل، کاربر، حالت روتینگ) |
...
**یونیت‌های systemd:**
- `cybergate-dnsrouter` — دیسپچر DNS روی پورت ۵۳
- `cybergate-socks5` — پراکسی SOCKS5 داخلی
- `cybergate-<tag>` — یکی به ازای هر تونل
...
سورس CyberGate: https://github.com/Iddy29/tzvpn/tree/main/cybergate
